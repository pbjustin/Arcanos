...
### Changed - Railway Compatibility & Security

- **Development host binding (BREAKING CHANGE):** The server now binds to `127.0.0.1` (localhost) by default in development mode instead of `0.0.0.0` (all interfaces). This improves security by preventing unintended network exposure during local development.
  - **Migration:** If you need network access in development (e.g., testing from other devices, Docker containers, or WSL2), set the `HOST` environment variable to `0.0.0.0` in your `.env` file.
  - **Production:** No change—production deployments continue to use `0.0.0.0` by default.
  - **Override:** Set `HOST=0.0.0.0` in your environment to restore the previous behavior in any environment.
...
- **Centralized defaults:** Port default (8080) and GPT-5 model fallbacks use `APPLICATION_CONSTANTS`; `.env.example` documents PORT and Railway GraphQL vars.
- **Trinity smoke test:** Added `tests/trinity.test.ts` asserting `TrinityResult` shape for dry-run path.
