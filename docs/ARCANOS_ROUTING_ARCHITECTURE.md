...
The ARCANOS system now implements a sophisticated routing architecture that ensures ALL tasks are processed through the fine-tuned ARCANOS model (`REDACTED_FINE_TUNED_MODEL_ID`) while conditionally leveraging GPT-5 for complex reasoning.
...
📊 [ROUTING SUMMARY] ARCANOS: REDACTED_FINE_TUNED_MODEL_ID | GPT-5 Used: true
```
...
```json
{
  ...
  "module": "REDACTED_FINE_TUNED_MODEL_ID",
  "activeModel": "REDACTED_FINE_TUNED_MODEL_ID",
  "fallbackFlag": false,
  "routingStages": [
    "ARCANOS-START:REDACTED_FINE_TUNED_MODEL_ID",
...
### Model Priority
1. `REDACTED_FINE_TUNED_MODEL_ID` (Primary ARCANOS model)
