...
- Only the separate credential-free job checks out the opted-in PR head, and it
  ...
  current 128-request probe with both network flags from a separate, clean
...
| `ARCANOS_BACKSTAGE_NOTION_PARTITIONS_JSON` | Optional; identical closed version-1 envelope on web and worker during partition validation | Declares bounded shards using stable keys, unique roots, retrieval tiers, required policy, scope/category tags, and finite capacity. The canonical semantic digest is independent of its operator generation. This additive configuration does not replace the monolithic authority latch. |
| `ARCANOS_BACKSTAGE_NOTION_PARTITIONED_INDEX_MODE` | Optional; identical rollout control on web and worker; defaults to `monolith` | Accepts only exact `monolith`, `shadow`, or `partitioned`. Absent or invalid values resolve to `monolith`. Keep `monolith` until the partitioned shadow has been validated; no production cutover is implied by configuring shard definitions. |
