...
| `ARCANOS_GPT_ACCESS_SCOPES` | Required for protected GPT access | Grant only the scopes needed by the intended operations. Gaming source ingestion/status uses `gaming.sources.write,gaming.sources.read`; async job submission and result retrieval use `jobs.create,jobs.result`; add other read, recovery, or capability scopes only when intentionally enabled. |
