...
- Only the separate credential-free job checks out the opted-in PR head, and it
  ...
  that adds the subtree continuity selector, that trusted lifecycle run can
  therefore remain on the pre-merge 121-request contract while the exact
  PR-head runner contains 122 requests. After the lifecycle reports the exact
  preview hosts, execute the 122-request probe with both network flags from a separate, clean,
  detached checkout of the revalidated exact PR head. This supplemental run
  covers the new web subtree-continuity fixture; the trusted contract already
  covers the worker generation-contract denial;
