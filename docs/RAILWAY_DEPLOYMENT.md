...
5. Railway detects `railway.json` and configures build/deploy commands automatically.
...
| --- | --- | --- |
| `OPENAI_MODEL` | — | Primary model selection (first in chain). |
| `AI_MODEL` | `gpt-4-turbo` | Legacy model variable used by config/worker defaults. |
...
| `ARC_LOG_PATH` | `/tmp/arc/log` | Explicitly set in Railway variables. |
...
The backend will build `DATABASE_URL` from `PG*` when needed.
...
3. Monitor build logs in the Railway dashboard (Deployments tab).
...
- `/railway/healthcheck` - Railway-focused diagnostic report
...
**Solutions:**
- Verify `OPENAI_API_KEY` is set and valid (not the placeholder).
