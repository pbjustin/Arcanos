...
- Only the separate credential-free job checks out the opted-in PR head, and it
  ...
  harness and contract come from the trusted default-branch checkout. For this
  PR, that trusted lifecycle run can therefore remain on the pre-merge
  122-request contract while the exact PR-head runner contains 123 requests.
  The existing review-completion selector silently executes the new
  production-shared compact-retry assertion without changing its trusted
  response shape. After the lifecycle reports the exact preview hosts, execute
  the 123-request probe with both network flags from a separate, clean checkout
  of the revalidated exact PR head. This supplemental run explicitly covers the
  new compact-retry fixture; the trusted contract already covers the worker
  generation-contract denial;
