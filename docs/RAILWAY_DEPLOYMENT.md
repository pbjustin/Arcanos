...
- Successful web `/readyz` requests also execute a sealed DAG metrics retention fixture over the same pure recorder core used by the production metrics adapter. The fixture checks retained scalar state before snapshots at two workload checkpoints, latest values including zero, counter/gauge and synthetic health-sink compatibility, detached snapshots, and independent recorder instances. Only a passing fixture emits `x-arcanos-preview-dag-metrics-version: dag-metrics-retention/v1`; failure returns 503 without that marker. The exact-head verifier requires the marker on initial and final web readiness and reports `dagMetricsRetentionVerified: true`, without adding readiness requests or changing the existing response body. This proves the deployed compiled retention core with a synthetic sink. It does not run the normal task runner, logger, database, provider, queue, or active worker, or establish total process memory bounds. The worker-path regression and compiled-runtime artifact fixture provide separate evidence for the production adapter and job execution wiring.
...
- The sealed Gaming guide request also executes the production progression,
  ...
  preserves polite and negative-clause task anchors, selects checkpoint-specific
  evidence, keeps editions distinct, and distinguishes generation timeouts with
  and without selected evidence. The exact-head verifier requires
  `x-arcanos-preview-gaming-progress-recovery-version: gaming-progress-recovery/v1`
  and reports `gamingProgressRecoveryVerified`. Failed assertions withhold all
  Gaming success markers and return the fixed guide-contract error. The existing
  response body and 138-request matrix remain compatible with the trusted
  verifier. This is served component evidence with synthetic inputs; it does not
  run normal routes, stored-source resolution or ingestion, SQL, a live provider,
  cache, or an active worker.
- The same sealed Gaming guide request also executes the production-shared
  player-context validators, request/context lexical policy, stored evidence
  selector/formatter, adaptive prompt policy, grounded composer, and compact
  intake policy against server-owned synthetic adventure, boss/action, and
  ship-systems cases. The additive header is
  `x-arcanos-preview-gaming-guide-assistance-version: gaming-guide-assistance/v1`;
  the exact-head verifier requires it and reports `gamingGuideAssistanceVerified`.
  It checks optional-field propagation between pure validation seams, nested
  alias precedence, context origins/conflicts, depth/spoiler normalization,
  checkpoint-dependent evidence, specific boss/item focus, sanitized and budgeted
  headings, consistent source/revision/record provenance, compact intake scope
  and 500-token allocation, Markdown preservation, and fallback exclusion.
  A failed assertion withholds all Gaming proof headers and the success body.
  The request/body and 138-request plan stay compatible with the trusted base
  verifier; the reviewed PR-head verifier supplies the new explicit assertion.
  The normal BackendQueryAgent, configured retrieval wrapper, Trinity stages,
  provider adapter, logger, SQL, network acquisition, cache, and worker remain
  outside this contained fixture. Separate mocked public-path E2E tests exercise
  actual mapping and Trinity/provider orchestration; guarded PostgreSQL CI
  supplies real storage/search evidence. Neither proves live-model compliance.
- The same sealed Gaming guide request also executes the production
  durable-document chunker and pure stored-evidence selection/formatting core
  over a server-owned synthetic guide of approximately 590,000 characters and
  a bounded synthetic candidate pool. The exact-head verifier requires
  `x-arcanos-preview-gaming-durable-rag-version: gaming-durable-rag/v1` and
  reports `gamingDurableRagVerified`. The proof checks deterministic bounded
  chunks, late-document evidence selection, revision identity, explicit
  500-chunk partial coverage, Unicode boundaries, candidate selection and
  overlap diversification, context/citation bounds, and retained internal
  provenance. It also exercises the complete existing 8,000-character
  structured-build evidence bound and the independent Archive character/byte
  limits. The configured retrieval wrapper stays outside the contained graph;
  the normal service passes its runtime limits to the same pure selection core.
  The chunker retains its bounded event-loop yields through the file-specific
  `setImmediate` import from `node:timers/promises`.
  Any assertion failure withholds all Gaming success markers and the success
  body. The unchanged guide response and 138-request matrix remain compatible
  with the trusted default-branch verifier; the supplemental exact-head verifier
  requires the additive marker. Candidate records and lexical ranks are
  synthetic. This proves served production-core components, not PostgreSQL
  full-text candidate generation/ranking, SQL persistence or supersession,
  configured stored retrieval, repository/ingestion composition, active queue
  or worker behavior, Archive/HTTP/DNS acquisition, provider calls, or logger
  execution.
  The separate `tests/integration/gaming-durable-rag.pg18.integration.test.ts`
  suite runs through `npm run test:postgres-fencing` against CI's guarded
  disposable PostgreSQL 18 database and a unique schema. A passing exact-head
  run supplies real repository/full-text matching and ranking evidence for
  deep-document retrieval, unchanged revisions, supersession, historical
  reactivation, and active-source/game filtering. It does not use a Railway
  database, production schema, worker, or provider.
- The same sealed Gaming guide request executes the pure prompt assembly and
  grounded-guide response composition used by production, with fixed synthetic
  evidence, prompt resources, and backend answers. The exact-head verifier
  requires `x-arcanos-preview-gaming-guide-response-version:
  gaming-guide-response/v1` and reports `gamingGuideResponseVerified`. The
  ...
  remain outside the preview. This is served component evidence, not live model
...
- Before returning the unchanged sealed Gaming guide response, the web preview
  ...
  verifier requires `x-arcanos-preview-gaming-archive-grounding-version:
  gaming-archive-grounding/v1` and reports `gamingArchiveGuideEvidenceVerified`.
  This preserves the trusted base verifier's response contract and request
  matrix. The fixture checks document provenance and selection, ambiguity and
  resource rejection, evidence diagnostics, the supplied-guide error gate, and
  fallback outcome classification. Any failed assertion prevents the proof
  header and success response. It does not call Archive.org, the real URL
  preparation/DNS/HTTP/extraction path, the ranker, the complete Gaming pipeline,
  a model provider, stored retrieval, or a structured logger sink. Those
  composition boundaries have separate mocked regression coverage; the sealed
  deployed result is served production-core evidence, not live guide ingestion.
...
- Web preview `/readyz` additionally runs the production attempt-usage collector
  ...
  requires the additive marker without adding readiness requests.
...
- The sealed web preview also exposes a fixed, stateless Tutor MCP mock at
  ...
  `2025-03-26` and the canonical Tutor input/output schemas. The answer is
  explicitly labelled `metadata.generation: "mock"`; it carries no backend
  memory or saved progress. The existing credential-carrier denial and 4 KiB
  pre-parser boundary still apply: Authorization, cookies, session headers,
...
- Only the separate credential-free job checks out the opted-in PR head, and it
  ...
  current 156-request PR-head probe with both network flags from a separate, clean
...
- The [iOS Swift HTTPS proof](../clients/ios/README.md#supplemental-swift-https-preview-proof)
  ...
  pre-Tutor 138-request matrix. Results prove real HTTPS client-to-fixture
...
- Worker `/readyz` remains `503` until database bootstrap,
  ...
  Configured Notion authority also requires a fast structural validation of its
  closed root mapping and dedicated purpose-bound token. Malformed required
  configuration is fatal, but a full Notion crawl, snapshot freshness, a busy
  ...
  partition-writer mode, and exact `partitioned` keeps those writers frozen.
...
the same exact SHA to the web service. Worker-first ordering avoids a new web
  ...
  separate rollout hold and stopped/drained procedure. Railway activation uses
  worker `/readyz` for process readiness only: a configured Backstage Notion
  heading/index upgrade continues asynchronously through the repository-owned
  lease and activation fence. It does not delay worker `SUCCESS` or the compatible
  web upload, while protected booking remains unavailable until the bounded
  authority status is `current_complete`.
...
### Decoupled Notion authority rollout
...
the authority crawl asynchronously. Database leases prevent overlapping active
writers; shutdown aborts the old attempt, its partial candidate cannot activate,
and a replacement may acquire the lease after expiry and perform a complete new
cycle. Protected booking remains unavailable while authority is `syncing`,
`last_known_good`, or `unavailable`; a newly issued request succeeds only after
durable status becomes `current_complete`. Already-claimed protected generation
that encounters unavailable authority returns the fixed terminal unavailable
envelope and is not silently replayed against later or stale data.

After the worker and web roles report the same combined SHA, observe the
authenticated, bounded monolith-authority status rather than triggering a
manual sync. Verify protected generation fails before current activation and
succeeds afterward without restarting either role. A later failed refresh may
preserve explicitly labeled continuity last-known-good behavior but cannot make
protected booking official.

The tracked start command uses
`node scripts/start-railway-service-with-integrity.mjs` and does not require
`notion-authority-root-probe.mjs`, `notion-root-status-diagnostic.mjs`, or
`notion-wiki-root-compat.mjs`. Those files are inert transition shims only for a
stale dashboard fallback. Clearing that fallback is a separately authorized
Railway configuration change; delete the shims only after both effective and
fallback commands have been re-read without preload references.
