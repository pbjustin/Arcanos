...
- The Backstage `continuity-query-contract` selector executes the production-shared cursor-shape preflight, sampled/exhaustive and compact-retry policy prompts, prompt assembly, and public response projection over server-owned sealed input. A paired worker request proves that the passive worker denies the generation-contract path with the contained 404. This is component evidence only: it does not invoke the canonical authenticated route, sign or verify a cursor MAC, execute SQL pagination, read live PostgreSQL or Notion data, or call OpenAI or any other model provider.
...
- Only the separate credential-free job checks out the opted-in PR head, and it
  ...
  harness and contract come from the trusted default-branch checkout. For a PR
  that adds the continuity selector, that trusted lifecycle run can therefore
  remain on the pre-merge 119-request contract while the exact PR-head runner
  contains 121 requests. After the lifecycle reports the exact preview hosts,
  execute the 121-request probe with both network flags from a separate, clean,
  detached checkout of the revalidated exact PR head. This supplemental run
  covers the new web continuity fixture and worker generation-contract denial;
  it is credential-free and has no Railway create, ownership, or cleanup
  authority.
