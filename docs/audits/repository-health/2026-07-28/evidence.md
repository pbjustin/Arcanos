...
drift tests on exact Node `24.18.1` and npm `11.16.0`. An explicit
`packageManager` field is intentionally omitted: the selected Node release
already bundles the exact npm baseline, while Railpack would otherwise activate
a moving Corepack release. The candidate does not alter application behavior or
upgrade unrelated production dependency versions.

The npm 11 install-script contract explicitly approves
`@prisma/client@5.22.0`, `msgpackr-extract@3.0.3`, and
`unrs-resolver@1.11.1`. The router override retains exact
`path-to-regexp@8.4.0` from the same upstream tag while using its published
prebuilt registry artifact instead of the prepare-dependent codeload source.
Exact npm reported no unreviewed install scripts; router loading, Prisma 5.22
generation/client loading, msgpackr native acceleration and round-trip, and
unrs native resolution passed on Windows x64.

| Local evidence | Result |
| --- | --- |
| Clean install and locks | `CI=true npm ci --no-audit --no-fund --foreground-scripts` installed 897 packages under exact Node/npm. Manifest and lockfile hashes were stable across the install; all three approved scripts ran and `npm approve-scripts --allow-scripts-pending` reported none. Root, worker, and standalone-runtime locks were generated by npm `11.16.0`, not hand-edited |
| Static, build, focused, and documentation checks | Type-check, lint (zero errors and 76 existing warnings), root build, workers build, Railway validation, the seven-suite 146-test focused version/workflow set, standalone runtime integration (7 suites / 75 tests), root integration (14 suites / 149 passed, 85 skipped without disposable services), documentation audit, documentation-link tests, and `git diff --check` passed under exact Node/npm |
| Broad Jest boundary | Aggregate completion is indeterminate, not green. A single-process root attempt exhausted roughly 4 GiB after 480 seconds. The bounded split reached 621 passing suites / 9,177 passing tests but retained the pre-existing GPT-OSS Windows-CRLF fixture failure and one Phase 2E `bash` child timeout. A non-GPT rerun reached 621 passing suites / 9,170 passing tests with only that child timeout; a further bounded rerun reached 620 passing suites / 9,157 passing tests with only a 20-second runtime-diagnostics timeout. The Phase 2E suite passed 13/13 focused and runtime diagnostics passed 9/9 focused, so both aggregate timeouts remain load-sensitive indeterminate evidence rather than focused regressions. The GPT-OSS fixture is excluded from this non-GPT slice |
| Compiled contained web startup | Exact Node `24.18.1` started `dist/start-native-pr-preview.js` with its credential-empty child environment; `/readyz` returned ready and the six-case MCP body-cap fixture passed. No child remained |
| Compiled keyless worker startup | The real emitted `jobRunner.run()` executed through `--import ./scripts/register-esm-loader.mjs` with only database/bootstrap and readiness imports replaced in memory. Database bootstrap occurred once; configured universes and provider reloads remained zero; the OpenAI adapter stayed uninitialized; fetch count was zero; execution stopped before consumer slots or job claiming |
