...
| `ARCANOS_BACKSTAGE_BOOKER_ACCESS_TOKEN` | No; protected Backstage Booker Custom GPT operations on the web service only | none | Dedicated purpose-bound Bearer credential for exactly `GET /gpt-access/capabilities/v1/backstage-booker/universes/{universeId}`, its fixed `/storyline-summary` exact-key leaf, plus `POST /gpt-access/capabilities/v1/backstage-booker/run` with `upsertStoryline` or `appendCanonBeat`. It must be 32–4096 visible ASCII characters with no whitespace or placeholder form and distinct from every other application credential. Configure the same value only on the web service and in the Backstage Booker Custom GPT Action's API Key/Bearer field. Generic GPT Access and other Backstage routes reject it. |
