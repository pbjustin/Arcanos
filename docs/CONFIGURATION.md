...
| `ARCANOS_GPT_ACCESS_SCOPES` | Yes for `/gpt-access/jobs/create`, capability discovery, capability runs, and worker recovery | all recognized read/control scopes are granted when unset, except `jobs.create`, `capabilities.read`, `capabilities.run`, and `workers.recover` remain denied unless explicitly listed | Comma-separated generic gateway scope allowlist. The Backstage Booker exact-ID universe read and dedicated canon write, plus Gaming source lifecycle Actions, use their own purpose-bound credentials rather than these scopes. `capabilities.read` covers generic capability metadata, not stored Backstage universe content. Include `jobs.create,jobs.result` for protected async Trinity execution; include `workers.recover` only for confirmed worker recovery dispatch; include `capabilities.run` only with a matching `MCP_ALLOW_MODULE_ACTIONS` allowlist and confirmation. |
...
`ARCANOS_BACKSTAGE_BOOKER_ACCESS_TOKEN` for the exact-ID
`GET /gpt-access/capabilities/v1/backstage-booker/universes/{universeId}` read
and the exact `POST /gpt-access/capabilities/v1/backstage-booker/run` canon
write. The read is non-consequential, skips generation and module dispatch,
and returns a bounded PostgreSQL projection with explicit truncation metadata.
...
its `capabilities.read` scope do not authorize this domain-data read.
...
| `ARCANOS_BACKSTAGE_BOOKER_ACCESS_TOKEN` | commented placeholder | Web-service-only dedicated API Key/Bearer credential for the exact-ID Backstage universe read and the exact canon Action route with only `upsertStoryline` or `appendCanonBeat`. It is distinct from generic GPT Access and control-plane credentials and must never appear in the schema, instructions, chat, source, or logs. |
