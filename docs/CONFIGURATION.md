...
`BOOKER_WORKER_TOKEN_LIMIT` controls only authenticated protected queued structured generation. It defaults to `6000`, clamps to `4000`-`8000`, and is further constrained by the finite primary provider-stage budget: less than 45 seconds retains `2400`, 45-60 seconds permits at most `4000`, 60-75 seconds at most `5000`, and 75 seconds or more permits the configured value. The extended policy is available only for the known GPT-5.1 and GPT-5.6 Sol/Terra/Luna request contracts. Unsupported model identifiers fail safely to `2400`. Genuine compact direct-answer, bounded-review, query-continuity, and synchronous rollback requests keep their smaller established budgets. A direct-answer cue does not demote a queued production booking or Notion-authoritative booking from structured capacity, and `card`/`cards` are not generic compact item-count nouns. The selected cap is passed through the installed SDK as the compatible Responses `max_output_tokens` field, and provider `incomplete`/`max_output_tokens` metadata remains a failure signal rather than successful booking text. Every attempt receives a server-owned instruction to finish all requested sections within the selected finite budget.
...
existing Builder Action. Continuity and booking answer generation make at most one
compact retry only for provider max-output exhaustion when finite recovery
budget remains available, reuse the same retrieval and budget, do not retry other provider failures, and return a sanitized
...
| `BOOKER_WORKER_TOKEN_LIMIT` | `6000` (commented) | Protected queued structured-generation budget, clamped to `4000`-`8000` and further limited by the compatible model and remaining finite provider-stage tier. It does not enlarge continuity, genuine compact, review, unsupported-model, or synchronous rollback calls; direct wording on a production booking remains structured capacity. |
