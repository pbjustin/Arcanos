...
| `ARCANOS_GPT_ACCESS_SCOPES` | Yes for `/gpt-access/jobs/create`, Gaming source writes, capability discovery, capability runs, and worker recovery | all recognized read/control scopes are granted when unset, except `jobs.create`, `capabilities.read`, `capabilities.run`, `gaming.sources.write`, and `workers.recover` remain denied unless explicitly listed | Comma-separated gateway scope allowlist. Include `gaming.sources.read,gaming.sources.write` for the narrow Gaming ingestion/status Actions; include `jobs.create,jobs.result` for protected async Trinity execution; include `workers.recover` only for confirmed worker recovery dispatch; include `capabilities.read` for discovery and `capabilities.run` only with a matching `MCP_ALLOW_MODULE_ACTIONS` allowlist and confirmation. |
...
| `ARCANOS_GPT_ACCESS_SCOPES` | Yes for job creation, Gaming source writes, capability discovery, capability runs, and worker recovery | all recognized scopes are granted when unset, except `jobs.create`, `capabilities.read`, `capabilities.run`, `gaming.sources.write`, and `workers.recover` remain denied unless explicitly listed | Scope allowlist. Use `gaming.sources.read,gaming.sources.write` for Gaming ingestion/status; use `runtime.read,workers.read,queue.read,jobs.create,jobs.result,diagnostics.read` for the protected async Trinity flow; add `workers.recover` only for confirmed worker recovery dispatch; add `capabilities.read` for discovery and `capabilities.run` only with `MCP_ALLOW_MODULE_ACTIONS` and confirmation. |
...
| `ARCANOS_GPT_ACCESS_SCOPES` | commented scope list | Gateway scope allowlist. `jobs.create`, `capabilities.read`, `capabilities.run`, `gaming.sources.write`, and `workers.recover` must be explicit before they enqueue, discover, execute capability work, mutate Gaming sources, or recover workers. |
