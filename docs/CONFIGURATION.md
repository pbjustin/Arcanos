...
| `ARCANOS_BACKSTAGE_BOOKER_ACCESS_TOKEN` | No; only for the protected Backstage Booker Custom GPT operations and authenticated Notion-backed generation on the web service | none | Dedicated exact 32–4096-character visible-ASCII Bearer credential for the protected read/write lane and request-local authorization of private Notion-derived context on canonical generation. For a Notion-authoritative universe, missing/invalid authentication fails generation closed instead of exposing either the private index or quarantined legacy state. Configure it on the web service and as API Key/Bearer authentication in the Backstage Booker Custom GPT Action only; do not set it on workers. |
...
| `ARCANOS_BACKSTAGE_NOTION_PARTITIONS_JSON` | No; identical value on web and worker when validating the partitioned index | none | Additive closed version-1 envelope containing an operator generation and bounded universe/shard definitions. Stable lowercase `shardKey` values are independent of display names. Each shard declares a unique root UUID, `hot`/`cold`/`archive` retrieval tier, required/optional behavior, sorted scope/category tags, and explicit finite page, chunk, depth, and content limits. Unknown fields, duplicate universe/shard/tag/root identities, malformed values, or excessive cardinality invalidate the complete envelope. Its canonical semantic SHA-256 digest is separate from the operator generation. |
| `ARCANOS_BACKSTAGE_NOTION_PARTITIONED_INDEX_MODE` | No; identical value on web and worker | `monolith` | Exact rollout mode: `monolith`, `shadow`, or `partitioned`. Absent, padded, differently cased, or unknown values resolve to `monolith` with non-sensitive validity metadata. Keep `monolith` during initial shadow validation; this flag does not weaken the durable authority latch. |
...
`BOOKER_WORKER_TOKEN_LIMIT` controls only authenticated protected queued structured generation. It defaults to `6000`, clamps to `4000`-`8000`, and is further constrained by the finite primary provider-stage budget: less than 45 seconds retains `2400`, 45-60 seconds permits at most `4000`, 60-75 seconds at most `5000`, and 75 seconds or more permits the configured value. The extended policy is available only for the known GPT-5.1 and GPT-5.6 Sol/Terra/Luna request contracts. Unsupported model identifiers fail safely to `2400`. Compact direct-answer, bounded-review, query-continuity, and synchronous rollback requests keep their smaller established budgets. The selected cap is passed through the installed SDK as the compatible Responses `max_output_tokens` field, and provider `incomplete`/`max_output_tokens` metadata remains a failure signal rather than successful booking text. Every attempt receives a server-owned instruction to finish all requested sections within the selected finite budget.
...
`ARCANOS_BACKSTAGE_BOOKER_ACCESS_TOKEN` on its
continuity-query/generation/simulation operation and accepts it exclusively for
private Notion provenance and the exact
...
supplement. Schema `1.5.0` has five Builder operations, materializes the nested
public payload, and adds the protected async-result read; it must be re-imported. Configure
...
lowercase bounded identifiers. Root UUIDs are globally unique in one envelope,
and duplicate universe IDs, shard keys, or tags are rejected rather than
...
so operators can warn safely. Keep `monolith` through initial shadow validation.
Neither the partition envelope nor this read-index mode downgrades the durable
one-way Notion authority latch or restores legacy reads and writes.
...
or raw Notion page IDs. Deploy schema 1.5.0 before re-importing it into the
...
| `ARCANOS_BACKSTAGE_BOOKER_ACCESS_TOKEN` | No; protected Backstage Booker Custom GPT operations and authenticated private context on web | none | Dedicated purpose-bound Bearer credential for the protected lane and Notion-derived continuity queries/generation. Legacy supplement remains optional; mapped authority queries and generation require it and fail closed when absent. Configure only on web and in the existing GPT Action. |
...
| `ARCANOS_BACKSTAGE_NOTION_PARTITIONED_INDEX_MODE` | No; identical on web and worker | `monolith` | Exact `monolith`, `shadow`, or `partitioned`; absent or invalid values remain monolithic. |
...
| `BOOKER_WORKER_TOKEN_LIMIT` | `6000` (commented) | Protected queued structured-generation budget, clamped to `4000`-`8000` and further limited by the compatible model and remaining finite provider-stage tier. It does not enlarge continuity, compact, review, unsupported-model, or synchronous rollback calls. |
...
| `ARCANOS_BACKSTAGE_BOOKER_ACCESS_TOKEN` | commented placeholder | Web-service-only dedicated API Key/Bearer credential for the exact Backstage reads/canon write and request-local authorization of scoped Notion continuity queries plus optional Notion context on canonical generation. It is distinct from generic GPT Access and control-plane credentials and must never appear in the schema, instructions, chat, source, or logs. |
