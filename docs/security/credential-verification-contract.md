...
| O-26 | Backstage Booker Action authentication: `src/services/backstageBookerAccessAuth.ts`; pre-parser boundary `src/services/backstageBookerHttpBoundary.ts`; leaf-route defense in `src/routes/gpt-access.ts`; optional generation provenance in `src/services/backstageNotionEnrichmentAuthorization.ts` | Exactly one `Authorization` header containing case-sensitive `Bearer <opaque-value>` with one space and a 32–4096-character visible-ASCII, no-whitespace token. Duplicate, missing, malformed, and incorrect credentials fail closed on protected reads/writes. Dedicated protected matches exist only for exact `POST /gpt-access/capabilities/v1/backstage-booker/run` and the `/gpt-access/capabilities/v1/backstage-booker/universes` namespace; neither has a trailing-slash alias. Canonical `POST /gpt/backstage-booker` and its `backstage` alias perform the same comparison optionally: success establishes request-local authorization and sensitive-context-use provenance for Notion enrichment, while missing/invalid/unavailable authentication preserves the public non-Notion PostgreSQL/process-fallback behavior. | Optional web-service-only `ARCANOS_BACKSTAGE_BOOKER_ACCESS_TOKEN`, resolved per request as a trimmed, non-placeholder purpose-bound credential with registry collision protection. It is configured only on the web service and in the Backstage Booker Custom GPT Action; workers do not receive it. The separate outbound Notion provider token is deliberately excluded from the inbound application-auth registry and is rejected if it equals any configured member. | `timingSafeEqualOpaqueSecret` | The exact GET universe leaf provides a bounded snapshot, and its fixed `/storyline-summary` leaf provides one exact-key, fixed 4,000-code-point summary page with mandatory version fencing after page zero. Both are non-consequential PostgreSQL-only reads with no list operation, broad body parser, generic credential fallback, confirmation, generation, writes, memory fallback, or module dispatch. The exact canon POST remains limited to `upsertStoryline` and `appendCanonBeat`, applies the shared GPT rate limit, strict 256 KiB UTF-8 JSON body limit, and no-store policy before broad parsing, may bypass generic `capabilities.run` scope authorization, and still requires preflight, `MCP_ALLOW_MODULE_ACTIONS`, and service checks. Parser failures use bounded 400, 413, or 415 envelopes. Other actions fail with fixed `403 BACKSTAGE_BOOKER_ACCESS_ACTION_DENIED`; the credential cannot authorize another GPT Access route, while a generic bearer follows its unchanged scope and confirmation path only on the canon POST. Optional direct-route provenance can only enable bounded, read-only, server-mapped Notion context after a successful PostgreSQL read; it grants no route admission, mutation, arbitrary URL, caller-selected page, or provider-token access. The original caller directive—not Notion text—owns Trinity policy parsing, and enriched runs redact lineage content, suppress prompt-debug response content, and skip generic module-conversation transcript persistence while retaining non-content audit metadata. |
