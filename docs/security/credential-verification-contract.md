...
| O-26 | Backstage Booker Action authentication: `src/services/backstageBookerAccessAuth.ts`; pre-parser boundary `src/services/backstageBookerHttpBoundary.ts`; dedicated result adapter `src/routes/backstageBookerAsyncResult.ts`; leaf-route defense in `src/routes/gpt-access.ts`; generation provenance in `src/services/backstageNotionEnrichmentAuthorization.ts` | Exactly one `Authorization` header containing case-sensitive `Bearer <opaque-value>` with one space and a 32–4096-character visible-ASCII, no-whitespace token. Duplicate, missing, malformed, and incorrect credentials fail closed on protected reads/writes. Dedicated protected matches exist only for exact `POST /gpt-access/capabilities/v1/backstage-booker/run`, exact `GET /gpt-access/capabilities/v1/backstage-booker/jobs/{jobId}/result`, and the `/gpt-access/capabilities/v1/backstage-booker/universes` namespace; none has a trailing-slash alias. Builder schema `1.6.0` declares the same bearer for every operation, including canonical `POST /gpt/backstage-booker`. That route verifies it into request-local provenance without granting another action; non-authoritative direct callers retain public non-Notion behavior, while a configured Notion-authoritative universe fails closed on missing, invalid, or unavailable provenance. | Optional web-service-only `ARCANOS_BACKSTAGE_BOOKER_ACCESS_TOKEN`, resolved per request as a trimmed, non-placeholder purpose-bound credential with registry collision protection. It authenticates one shared, server-owned, purpose-bound managed principal rather than a user, session, tenant, or universe, so new `1.6.0` job ownership remains stable across future token rotations. It is configured only on the web service and in the Backstage Booker Custom GPT Action; workers do not receive it. The separate outbound Notion provider token is deliberately excluded from the inbound application-auth registry and is rejected if it equals any configured member. | `timingSafeEqualOpaqueSecret` | The managed result GET is a non-consequential, bodyless, poll-only read of one protected public-GPT Booker job. It returns no dynamic job token or bearer stream, requires the shared managed principal, and therefore allows any valid credential holder with a known valid job UUID in that trust domain to poll the result. New jobs use the stable principal; during upgrade, only the exact current credential can also match its prior token-derived legacy owner, so pre-`1.6.0` result-retention and idempotency windows must drain before the first token rotation. The compatibility is read-only ownership checking and does not alias the old idempotency scope; callers retain `jobId` and never resubmit legacy work as a lookup. The exact GET universe leaf provides a bounded snapshot, and its fixed `/storyline-summary` leaf provides one exact-key, fixed 4,000-code-point summary page with mandatory version fencing after page zero. Both are non-consequential PostgreSQL-only reads with no list operation, broad body parser, generic credential fallback, confirmation, generation, writes, memory fallback, or module dispatch. Notion-authoritative scopes return a nonretryable legacy-read quarantine instead. The exact canon POST remains limited to `upsertStoryline` and `appendCanonBeat`, applies the shared GPT rate limit, strict 256 KiB UTF-8 JSON body limit, and no-store policy before broad parsing, may bypass generic `capabilities.run` scope authorization, and still requires preflight, `MCP_ALLOW_MODULE_ACTIONS`, and service checks; authority mode denies all legacy writes in service and database layers. Parser failures use bounded 400, 413, or 415 envelopes. Other actions fail with fixed `403 BACKSTAGE_BOOKER_ACCESS_ACTION_DENIED`; the credential cannot authorize another GPT Access route, while a generic bearer follows its unchanged scope and confirmation path only on the canon POST. Direct-route provenance can enable only bounded, read-only, server-mapped Notion context. Authority retrieval uses one verified immutable RAG snapshot and never falls back to legacy state; retrieved text has no instruction, tool, policy, or persistence authority. The original caller directive owns Trinity policy parsing, and Notion-backed runs redact lineage content, suppress prompt-debug response content, skip generic module-conversation transcript persistence, and force provider `store: false` while retaining non-content audit metadata. Heavy generation attests this already-verified provenance into an identity-bound encrypted queue payload; the worker receives neither the bearer nor authorization headers, and terminal output remains encrypted until the managed bearer result read or the unchanged generic job-read capability succeeds. |

O-26 has an additional fail-closed authority mode when
`ARCANOS_BACKSTAGE_NOTION_AUTHORITY_ROOTS_JSON` is present. For an exact mapped
universe, the same verified request-local bearer is mandatory before private
RAG retrieval; missing/invalid auth never falls back to legacy state. All six
service mutation entries and the generate-and-save route deny before side
effects, while database triggers defend the nine legacy tables after
activation. The two protected PostgreSQL GETs return a fixed quarantined 409.
Only the worker holds the outbound Notion token and follows the fixed configured
root; web reads one universe/snapshot-scoped derived index. Notion excerpts have
factual but zero instruction authority, and sensitive provider storage,
transcript, debug, lineage, judged-feedback, and HRC-cache controls remain
mandatory. A malformed present authority mapping blocks mutations globally
until corrected rather than silently reopening writes.

Schema `1.6.0` keeps the existing Action authentication grammar unchanged. The opt-in read
scope introduced in `1.4.0` still defaults to one exact page; `"subtree"` includes
that parent and descendants but no sibling, supports a blank navigation parent,
and rejects `sectionPath`. Subtree `relevant` retrieval remains bounded and
diversified; `complete_scope` remains synchronous and request-local and binds
its cursor to the request, scope kind, and active snapshot. Version-2 cursors
fail closed after rollout and must restart without a cursor. Public responses
never expose raw Notion page IDs; only subtree responses identify
`resolvedScope.scopeKind: "subtree"` and add page coverage beside chunk
coverage. The fifth operation reads pending or terminal async results through
the managed bearer namespace using `jobId` plus optional bounded waiting. Its
accepted and result envelopes expose the same managed poll URL and omit
`jobReadToken`, `jobReadTokenHeader`, and `stream`; generic capability-token
status/result/stream routes remain separate. The exact current credential maps
to the stable managed principal for new work and, during upgrade only, its own
token-derived legacy owner. Arbitrary and previous tokens are never accepted.
The backend and Builder schema cannot be changed independently while generation
remains available: the old `1.5.0` Action needs the dynamic job capability that
the new backend omits, while `1.6.0` calls a managed route absent from the old
backend. Enter a generation maintenance window, stop all new generation
(including explicit async/idempotent requests), drain legacy jobs and required
terminal re-reads, remove every old web replica from traffic, deploy and verify
the backend, and then immediately re-import its live schema `1.6.0` before
reopening generation. The saved bearer remains only in the encrypted Action
authentication field. Rollback is likewise unavailable while stable-principal
jobs must remain readable: stop generation and wait for or verify their complete
retention drain before restoring an older backend or Builder schema.
