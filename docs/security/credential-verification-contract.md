...
Schema `1.5.0` keeps this authentication boundary unchanged. The opt-in read
...
coverage. The fifth operation reads a terminal async result only with the
per-job capability returned by the accepted generation response. Deploy the
backend before re-importing schema `1.5.0` into Builder; the saved bearer remains
only in the encrypted Action authentication field.
