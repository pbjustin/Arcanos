...
Schema `1.4.0` keeps this authentication boundary unchanged while adding an
opt-in read scope. `scopeKind` defaults to one exact page; `"subtree"` includes
...
coverage. Deploy the backend before re-importing schema `1.4.0` into Builder;
the saved bearer remains only in the encrypted Action authentication field.
