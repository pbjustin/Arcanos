...
The result transaction locks the immutable command run set, plan, and current actions in the deterministic order defined below. Before any terminal write, it verifies owner, instance, realm, plan/action membership, stored snapshot identity, result idempotency, the locked plan generation, current direct-database capability grants, and every run's immutable action snapshot. It then conditionally changes only the bound run from `RUNNING` to `SUCCEEDED` or `FAILED`, records the first result-key hash, result fingerprint, and opaque acceptance receipt, and appends one terminal event. A submitted snapshot mismatch, generation change, current action change, or capability change is rejected with no terminal mutation; one bounded safe rejection event may be appended for the authenticated owner.

It then aggregates only the immutable run set belonging to that command. Plan completion is a compare-and-swap from the expected `in_progress` generation. A concurrently blocked, expired, failed, completed, or otherwise incompatible plan makes the submission stale or causes the final compare-and-swap to fail, rolling back the terminal write. The operation never initiates execution, creates a run, touches a sibling run's terminal fields, or re-evaluates owner selection.
...
Claim and start compare the locked current action, plan generation, and direct-database capability grant with the stored immutable snapshot before returning or authorizing executable work. Capability revocation or reassignment supersedes the affected unstarted command run set; it never relies on the agent-registry cache. Result acceptance repeats those generation, capability, and command-wide snapshot checks before its terminal compare-and-swap. Python treats the snapshot ID as opaque, executes exactly the returned snapshot, and echoes only the ID; it does not recompute a digest. A changed plan/action before start cannot execute. A change detected after a local side effect causes result rejection and fail-closed acknowledgement; the durable local journal retains the unresolved outcome for operator recovery and the backend never fabricates success for the changed plan.
...
| Result races a concurrent block or plan change | Locked generation/snapshot validation plus plan-state CAS | Stale result is rejected and the terminal write is rolled back |
