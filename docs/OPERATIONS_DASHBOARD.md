...
| Worker health | `worker_health_status`, `worker_heartbeat_age_ms`, `worker_stale_workers` | heartbeat age above `2 * JOB_WORKER_STALE_AFTER_MS`; stale workers > 0 for 2 checks |
...
| AI failures/timeouts | `ai_calls_total{outcome!="ok"}`, `ai_timeouts_total`, `dependency_failures_total`, `dependency_timeouts_total` | error ratio above 5% for 10m; timeout spike |
| AI circuit breaker | `ai_circuit_breaker_state`, `ai_circuit_breaker_failures` | state `open` for 2 checks |
...
| Event throughput | `gpt_job_events_total`, `job_event_insert_failures_total`, `job_events_cleanup_rows_total` | sudden zero event rate while jobs are active; insert failures above 0 |
...
`gpt_job_timing_ms{phase="queue_wait"}` is the created-to-claimed queue latency, `gpt_job_timing_ms{phase="execution"}` is started-to-terminal execution latency, and `gpt_job_timing_ms{phase="end_to_end"}` is created-to-terminal latency. Claim-to-start is expected to be near-zero because claim sets `started_at` in the same database update.
