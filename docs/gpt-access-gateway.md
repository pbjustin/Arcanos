...
Troubleshooting `daemonReachable:false`: verify the web service has `ARCANOS_CLI_BRIDGE_ENABLED=true`, `ARCANOS_CLI_BRIDGE_URL` is loopback-only, the bridge token is present, the sandbox path exists, and the web service was redeployed after variable changes. Runtime logs should include routine checks such as `arcanos.daemon.health.checked`, `arcanos.cli.status.checked`, and `arcanos.cli.policy.read`. Durable GPT Access logs persist higher-value lifecycle and proposal events such as `daemon.started`, `arcanos.daemon.unreachable`, `arcanos.daemon.recovered`, `arcanos.cli.command.proposed`, `arcanos.cli.command.denied`, `arcanos.cli.patch.proposed`, and `arcanos.cli.patch.denied`; these can be checked with `/gpt-access/logs/query` without exposing bridge tokens, command text, patch text, output, prompts, provider payloads, or environment values.
