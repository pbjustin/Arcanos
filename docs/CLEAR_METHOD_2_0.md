...
## Overview
CLEAR Method 2.0 is Arcanos' governance-scoring framework for plan safety and operational quality. It evaluates plans across five principles and produces a deterministic decision (`allow`, `confirm`, or `block`) that downstream routes can enforce. The implementation is centered on `src/services/clear2.ts` and is consumed by plan creation, plan execution rechecks, and dedicated CLEAR endpoints. (`src/services/clear2.ts`) (`src/routes/plans.ts`) (`src/routes/clear.ts`)
...
### 1) Principle scoring
`computeClear2PrincipleScores` calculates each principle score from plan signals (action structure, confidence, rollback presence, capability/agent context), then clamps scores to `[0,1]`. (`src/services/clear2.ts`)

### 2) Composite score
`computeClear2CompositeScore` applies normalized weights to principle scores and returns a rounded composite score (3 decimal places). (`src/services/clear2.ts`)
...
- efficiency: `0.20`
- alignment: `0.20`
- resilience: `0.20` (`src/services/clear2.ts`)
...
1. **Plan governance auditing (CLEAR 2.0)**
   ...
   - Decision and score are persisted and logged, making plan lifecycle transitions auditable (`planned`, `awaiting_confirmation`, `approved`, `blocked`, etc.). (`src/stores/actionPlanStore.ts`) (`src/stores/actionPlanStore.ts`)
...
2. **Feedback auditing (`/audit`) with CLEAR scorecards**
   ...
   - This path uses scorecard-style CLEAR payload parsing (`scores`, composite/status) and includes explicit `//audit` annotations for branch assumptions and failure handling. (`src/routes/reinforcement.ts`) (`src/services/audit.ts`) (`src/services/clearScorecard.ts`)
...
- `GET /clear/:planId` fetches a persisted CLEAR score. (`src/routes/plans.ts`) (`src/routes/clear.ts`)

### Store-level usage
- `createPlan` computes CLEAR 2.0, persists the score, and derives initial plan status from decision.
- Status transitions remain constrained by decision (for example, blocked plans cannot be approved). (`src/stores/actionPlanStore.ts`) (`src/stores/actionPlanStore.ts`)
...
- **Audit traceability in reinforcement endpoints** (`/audit`) for post-execution feedback loops. (`src/core/logic/trinity.ts`) (`src/routes/reinforcement.ts`)
...
5. `POST /audit` can later register outcome feedback and forward CLEAR feedback externally for reinforcement analysis. (`src/routes/plans.ts`) (`src/stores/actionPlanStore.ts`) (`src/routes/reinforcement.ts`)
