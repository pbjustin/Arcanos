...
## Overview
CLEAR Method 2.0 is Arcanos' governance-scoring framework for plan safety and operational quality. It evaluates plans across five principles and produces a deterministic decision (`allow`, `confirm`, or `block`) that downstream routes can enforce. The implementation is centered on `src/services/clear2.ts` and is consumed by plan creation, plan execution rechecks, and dedicated CLEAR endpoints.【F:src/services/clear2.ts†L61-L248】【F:src/routes/plans.ts†L33-L295】【F:src/routes/clear.ts†L20-L71】
...
- **E — Efficiency**
- **A — Alignment**
- **R — Resilience**【F:src/services/clear2.ts†L3-L12】
...
1. **Plan governance auditing (CLEAR 2.0)**
   ...
   - Decision and score are persisted and logged, making plan lifecycle transitions auditable (`planned`, `awaiting_confirmation`, `approved`, `blocked`, etc.).【F:src/stores/actionPlanStore.ts†L61-L125】【F:src/stores/actionPlanStore.ts†L147-L192】
...
2. **Feedback auditing (`/audit`) with CLEAR scorecards**
   ...
   - This path uses scorecard-style CLEAR payload parsing (`scores`, composite/status) and includes explicit `//audit` annotations for branch assumptions and failure handling.【F:src/routes/reinforcement.ts†L36-L55】【F:src/services/audit.ts†L36-L266】【F:src/services/clearScorecard.ts†L50-L210】
...
- `GET /clear/:planId` fetches a persisted CLEAR score.【F:src/routes/plans.ts†L33-L295】【F:src/routes/clear.ts†L20-L71】

### Store-level usage
- `createPlan` computes CLEAR 2.0, persists the score, and derives initial plan status from decision.
- Status transitions remain constrained by decision (for example, blocked plans cannot be approved).【F:src/stores/actionPlanStore.ts†L61-L125】【F:src/stores/actionPlanStore.ts†L168-L188】
...
- **Audit traceability in reinforcement endpoints** (`/audit`) for post-execution feedback loops.【F:src/core/logic/trinity.ts†L166-L397】【F:src/routes/reinforcement.ts†L36-L55】
...
5. `POST /audit` can later register outcome feedback and forward CLEAR feedback externally for reinforcement analysis.【F:src/routes/plans.ts†L33-L295】【F:src/stores/actionPlanStore.ts†L61-L302】【F:src/routes/reinforcement.ts†L36-L55】
