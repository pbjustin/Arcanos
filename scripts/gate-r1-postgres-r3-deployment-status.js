#!/usr/bin/env node
/**
 * Purpose: Project the exact PostgreSQL R3 Railway deployment status without mutation.
...
} from './gate-r1-postgres-r3-source-activation.js';
import { GATE_R_POSTGRES_SERVICE_NAME } from './gate-r1-postgres-readiness.js';
...
export const GATE_R1_R3_DEPLOYMENT_MAX_OBSERVATIONS = 120;
export const GATE_R1_R3_DEPLOYMENT_POLL_INTERVAL_MS = 5_000;
export const GATE_R1_R3_DEPLOYMENT_DEADLINE_MS = 600_000;
const SAFE_FAILURES = new Set([
  'GATE_R1_R3_DEPLOYMENT_STATUS_AMBIENT_TOKEN_FORBIDDEN',
  'GATE_R1_R3_DEPLOYMENT_STATUS_ARGUMENT_INVALID',
  'GATE_R1_R3_DEPLOYMENT_STATUS_CLI_UNAVAILABLE',
  'GATE_R1_R3_DEPLOYMENT_ID_MISMATCH',
  'GATE_R1_R3_DEPLOYMENT_NOT_SUCCESSFUL',
  'GATE_R1_R3_DEPLOYMENT_POLL_TIMEOUT',
  'GATE_R1_R3_DEPLOYMENT_STOPPED',
  'GATE_R1_R3_DEPLOYMENT_STATUS_QUERY_FAILED',
  'GATE_R1_R3_DEPLOYMENT_STATUS_RESPONSE_INVALID',
  'GATE_R1_R3_DEPLOYMENT_TERMINAL_FAILURE',
...
function decodeBounded(value, maximumBytes) {
  if (!Buffer.isBuffer(value) || value.length > maximumBytes) {
    fail('GATE_R1_R3_DEPLOYMENT_STATUS_RESPONSE_INVALID');
  }
  try {
...
function resolveReadTimeout(deadlineAt, now) {
  ...
  if (!Number.isFinite(deadlineAt) || !Number.isFinite(current) || current >= deadlineAt) {
    fail('GATE_R1_R3_DEPLOYMENT_POLL_TIMEOUT');
  }
  return Math.max(1, Math.min(30_000, Math.floor(deadlineAt - current)));
}

export function projectGateR1PostgresR3DeploymentStatus(raw, { expectedDeploymentId } = {}) {
  if (!exactKeys(raw, ['id', 'name', 'deploymentId', 'status', 'stopped'])
      || raw.id !== GATE_R1_R3_POSTGRES_SERVICE_ID
      || raw.name !== GATE_R_POSTGRES_SERVICE_NAME
  ...
  if (raw.deploymentId === null && raw.status === null) {
    ...
    if (!knownStatus) fail('GATE_R1_R3_DEPLOYMENT_STATUS_RESPONSE_INVALID');
    if (raw.stopped) {
    ...
    } else if (TERMINAL_FAILURE_STATUSES.has(raw.status)) {
  ...
  if (expectedDeploymentId !== undefined
    ...
    fail('GATE_R1_R3_DEPLOYMENT_ID_MISMATCH');
  }

  return Object.freeze({
    code: 'GATE_R1_R3_DEPLOYMENT_STATUS_PROJECTED',
    ...
    serviceName: GATE_R_POSTGRES_SERVICE_NAME,
    stateCategory,
    status: 'PASS',
    stopped: raw.stopped
  });
...
export function runGateR1PostgresR3DeploymentStatus({
  railwayExecutable,
  ...
  if (typeof now !== 'function') {
    fail('GATE_R1_R3_DEPLOYMENT_STATUS_ARGUMENT_INVALID');
  }
  ...
  try {
    ...
    fail('GATE_R1_R3_DEPLOYMENT_STATUS_AMBIENT_TOKEN_FORBIDDEN');
  }
  ...
  try {
    ...
    fail('GATE_R1_R3_DEPLOYMENT_STATUS_CLI_UNAVAILABLE');
  }
  ...
  try {
    ...
    if (child?.error?.code === 'ETIMEDOUT') {
      fail('GATE_R1_R3_DEPLOYMENT_STATUS_TIMEOUT');
    }
    if (child?.error || child?.status !== 0) {
      fail('GATE_R1_R3_DEPLOYMENT_STATUS_QUERY_FAILED');
    }
    const stderr = decodeBounded(child.stderr, 256);
    const stdout = decodeBounded(child.stdout, 1024);
    if (stderr.length !== 0) fail('GATE_R1_R3_DEPLOYMENT_STATUS_RESPONSE_INVALID');
    let raw;
    try {
    ...
    return projectGateR1PostgresR3DeploymentStatus(raw, { expectedDeploymentId });
  } catch (error) {
...
export function waitForGateR1PostgresR3Deployment({
  readStatus = (expectedDeploymentId, timing) => runGateR1PostgresR3DeploymentStatus({
    expectedDeploymentId,
    ...timing
  }),
  now = () => performance.now(),
  ...
  const deadlineAt = startedAt + GATE_R1_R3_DEPLOYMENT_DEADLINE_MS;
  let expectedDeploymentId;
  for (let observation = 1;
    observation <= GATE_R1_R3_DEPLOYMENT_MAX_OBSERVATIONS;
    observation += 1) {
    if (now() >= deadlineAt) fail('GATE_R1_R3_DEPLOYMENT_POLL_TIMEOUT');
    let result;
    try {
    ...
    if (!exactKeys(result, [
      ...
      'serviceId', 'serviceName', 'stateCategory', 'status', 'stopped'
        ...
        || result.code !== 'GATE_R1_R3_DEPLOYMENT_STATUS_PROJECTED'
        || result.environmentId !== GATE_R1_R3_ENVIRONMENT_ID
        || result.projectId !== GATE_R1_R3_PROJECT_ID
        || result.serviceId !== GATE_R1_R3_POSTGRES_SERVICE_ID
        || result.serviceName !== GATE_R_POSTGRES_SERVICE_NAME
        || result.status !== 'PASS') {
      fail('GATE_R1_R3_DEPLOYMENT_STATUS_RESPONSE_INVALID');
    }

    if (result.stopped) fail('GATE_R1_R3_DEPLOYMENT_STOPPED');
    ...
    if (result.stateCategory === 'TERMINAL_FAILURE') {
      fail('GATE_R1_R3_DEPLOYMENT_TERMINAL_FAILURE');
    }
    if (!['NO_DEPLOYMENT', 'PENDING'].includes(result.stateCategory)) {
      fail('GATE_R1_R3_DEPLOYMENT_STATUS_RESPONSE_INVALID');
    }
    if (observation === GATE_R1_R3_DEPLOYMENT_MAX_OBSERVATIONS) {
      fail('GATE_R1_R3_DEPLOYMENT_POLL_TIMEOUT');
    }
    if (observedAt + GATE_R1_R3_DEPLOYMENT_POLL_INTERVAL_MS >= deadlineAt) {
      fail('GATE_R1_R3_DEPLOYMENT_POLL_TIMEOUT');
    }
    sleep(GATE_R1_R3_DEPLOYMENT_POLL_INTERVAL_MS);
  }
  fail('GATE_R1_R3_DEPLOYMENT_POLL_TIMEOUT');
}

export function requireGateR1PostgresR3DeploymentSuccess({
  deploymentId,
  readStatus = expectedDeploymentId => runGateR1PostgresR3DeploymentStatus({
    expectedDeploymentId
  })
} = {}) {
  ...
  if (result.stateCategory !== 'SUCCESS' || result.stopped
    ...
    fail('GATE_R1_R3_DEPLOYMENT_NOT_SUCCESSFUL');
  }
  return result;
}

export function parseGateR1PostgresR3DeploymentStatusArgs(argv) {
  if (Array.isArray(argv) && argv.length === 2
      && argv[0] === '--service-id' && argv[1] === GATE_R1_R3_POSTGRES_SERVICE_ID) {
    return Object.freeze({
  ...
  if (Array.isArray(argv) && argv.length === 4
      ...
      && argv[2] === '--service-id' && argv[3] === GATE_R1_R3_POSTGRES_SERVICE_ID) {
    return Object.freeze({
      deploymentId: undefined,
      operation: 'wait',
      serviceId: GATE_R1_R3_POSTGRES_SERVICE_ID
...
if (isMain) {
  try {
    const args = parseGateR1PostgresR3DeploymentStatusArgs(process.argv.slice(2));
    let result;
    if (args.operation === 'wait') {
      result = waitForGateR1PostgresR3Deployment();
    } else if (args.operation === 'verify-success') {
      result = requireGateR1PostgresR3DeploymentSuccess({
        deploymentId: args.deploymentId
      });
    } else {
      result = runGateR1PostgresR3DeploymentStatus();
    }
