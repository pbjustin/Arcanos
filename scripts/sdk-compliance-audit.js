 ...
 */
async function auditCodeQuality(result) {
 ...
 */
async function auditSDKCompliance(result) {
    ...
    for (const file of files) {
        const content = await fs.readFile(file, 'utf8');
        if ((content.includes('fetch') && content.includes('openai.com')) ||
 ...
 */
async function auditModules(result) {
        ...
        try {
            ...
            for (const file of files) {
                if (file.endsWith('.ts') || file.endsWith('.js')) {
                    ...
                    if (lineCount > 500) {
                        result.modules[moduleName].issues.push(`${file} has ${lineCount} lines (consider splitting)`);
 ...
 */
async function auditDeploymentReadiness(result) {
    ...
    // Check railway.json
    try {
        const railwayConfig = JSON.parse(await fs.readFile(path.join(projectRoot, 'railway.json'), 'utf8'));
...
function generateSummary(result) {
    ...
    const criticalIssues = !result.codeQuality.typeCheckPassed ||
    ...
    const convergence = result.codeQuality.typeCheckPassed &&
        result.codeQuality.lintPassed &&
        result.codeQuality.buildPassed &&
 ...
 */
async function saveComplianceReport(result) {
