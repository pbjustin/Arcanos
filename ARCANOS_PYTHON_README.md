...
The Arcanos backend exposes a fine-tuned GPT-5.2 workflow to Python workloads via
...
│   ├── client.py           # Reasoning helper enforcing GPT-5.2 usage
...
3. **Export required environment variables**:
   ...
   - `ARCANOS_FINE_TUNED_MODEL`: GPT-5.2 fine-tuned model identifier.
...
- **Strict model enforcement** – The class fails fast if `ARCANOS_FINE_TUNED_MODEL`
  is absent, ensuring GPT-5.2 reasoning never silently falls back.
...
| `ARCANOS_FINE_TUNED_MODEL` | ✅ | Fine-tuned GPT-5.2 model ID authorized for this repo. |
...
shippable Python interface for GPT-5.2 reasoning.
