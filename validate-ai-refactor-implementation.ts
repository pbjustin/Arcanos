...
import { dispatchEmail } from './src/workers/email/email-dispatcher';
...
async function testCoreAIService() {
  ...
  try {
    const messages = [
      ...
      { role: 'user' as const, content: 'Say hello and confirm you are using the arcanos-v1 model.' }
    ];
...
async function testStreamingFunctionality() {
  ...
  try {
    let streamedContent = '';
    const messages = [
      { role: 'system' as const, content: 'You are ARCANOS. Write a short 2-sentence test response.' },
      { role: 'user' as const, content: 'Please provide a brief test message to validate streaming works.' }
    ];
