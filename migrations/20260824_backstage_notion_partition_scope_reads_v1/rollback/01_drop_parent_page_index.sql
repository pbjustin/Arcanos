-- Destructive rollback. Quiesce scope readers first. The catalog guard and
-- ordinary DROP share one transaction so a wrong same-name object cannot race in.
...
BEGIN
  LOCK TABLE public.backstage_notion_shard_snapshot_pages
  ...
  SELECT
    class.oid, index_data.indrelid, class.relkind, index_data.indislive,
    index_data.indisunique, index_data.indisprimary, index_data.indisexclusion,
    method.amname, index_data.indnkeyatts, index_data.indnatts,
    index_data.indkey::TEXT, index_data.indcollation::TEXT,
    index_data.indclass::TEXT, index_data.indoption::TEXT,
  ...
  INTO
    index_oid, indexed_table_oid, relation_kind, index_live, index_unique,
    index_primary, index_exclusion, access_method, key_attribute_count,
    total_attribute_count, indexed_columns, index_collations, index_opclasses,
    index_options, index_predicate, key_definitions
  ...
  IF index_oid IS NULL THEN
    RETURN;
  END IF;
  IF expected_columns IS NULL
    OR expected_opclasses IS NULL
    OR indexed_table_oid IS DISTINCT FROM
      'public.backstage_notion_shard_snapshot_pages'::pg_catalog.regclass
    ...
    RAISE EXCEPTION 'scope-read parent index is unexpected; rollback refused'
      USING ERRCODE = '42804';
  END IF;

  EXECUTE 'DROP INDEX public.idx_backstage_notion_shard_snapshot_pages_parent';
END
$block$;
