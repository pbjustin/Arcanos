...
-- Quiesce readers before this atomic ordinary drop.
...
BEGIN
  ...
  SELECT
    class.oid, index_data.indrelid, class.relkind, index_data.indisvalid,
    index_data.indisready, index_data.indislive, index_data.indisunique,
    index_data.indisprimary, index_data.indisexclusion, method.amname,
    index_data.indnkeyatts, index_data.indnatts, index_data.indkey::TEXT,
    index_data.indcollation::TEXT, index_data.indclass::TEXT,
  ...
  IF index_oid IS NULL THEN
    RAISE EXCEPTION 'scope-read parent index is missing; recovery refused'
      USING ERRCODE = '55000';
  END IF;
  IF expected_columns IS NULL
    ...
    RAISE EXCEPTION 'scope-read parent index is unexpected; recovery refused'
      USING ERRCODE = '42804';
  END IF;
  IF index_valid IS DISTINCT FROM FALSE AND index_ready IS DISTINCT FROM FALSE THEN
    RAISE EXCEPTION 'scope-read parent index is healthy; recovery refused'
      USING ERRCODE = '55000';
  END IF;
