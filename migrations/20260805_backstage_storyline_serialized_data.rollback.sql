-- Remove only an unused, exactly verified storyline storage component.
-- Once any exact beat has been written, rollback is intentionally refused
-- because serialized_data is authoritative and the JSONB column is a placeholder.

DO $$
DECLARE
  serialized_data_attribute_number SMALLINT;
  ...
  storage_sequence_attribute_number SMALLINT;
  ...
  actual_constraint_validated BOOLEAN;
...
BEGIN
  ...
  INTO
    serialized_data_attribute_number,
  ...
  SELECT
    ...
    attribute.attinhcount
  INTO
    storage_sequence_attribute_number,
  ...
  IF serialized_data_type IS NULL AND storage_sequence_type IS NULL THEN
    RETURN;
  END IF;

  IF serialized_data_type IS NULL OR storage_sequence_type IS NULL THEN
    RAISE EXCEPTION
      'backstage storyline storage columns are only partially installed'
      USING ERRCODE = '42804';
  END IF;
  ...
  IF actual_constraint_oid IS NULL THEN
    RAISE EXCEPTION
      'backstage_story_beats_serialized_data_contract is missing'
      USING ERRCODE = '42804';
  END IF;
  ...
  IF actual_constraint_type <> 'c'
    ...
    OR NOT actual_constraint_validated
  ...
  IF EXISTS (
    ...
    WHERE serialized_data IS NOT NULL
       OR storage_sequence IS NOT NULL
  ) THEN
    RAISE EXCEPTION
  ...
  IF EXISTS (
    ...
    WHERE refclassid = 'pg_class'::regclass
      AND refobjid = 'backstage_story_beats'::regclass
      AND refobjsubid IN (
        serialized_data_attribute_number,
        storage_sequence_attribute_number
      )
      AND NOT (
        ...
        AND objid = actual_constraint_oid
      )
  ) THEN
    RAISE EXCEPTION
      'backstage storyline storage columns have unexpected dependent objects'
      USING ERRCODE = '2BP01';
  END IF;
  ...
  ALTER TABLE backstage_story_beats
    DROP COLUMN storage_sequence;

  ALTER TABLE backstage_story_beats
    DROP COLUMN serialized_data;
END
$$;
