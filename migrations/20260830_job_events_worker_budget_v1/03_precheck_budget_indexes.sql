-- Refuse same-name relation drift or an interrupted exact concurrent build.
...
BEGIN
  CREATE TEMP TABLE worker_budget_index_expected_guard (
    id UUID,
    job_id UUID,
    event_type TEXT,
  ...
  CREATE INDEX worker_budget_expected_group_guard
    ON pg_temp.worker_budget_index_expected_guard
      (stats_worker_id, event_type, occurred_at, id)
    WHERE event_type IN (
      'worker.budget.job_claim',
      'worker.budget.ai_provider_attempt'
    );
  CREATE UNIQUE INDEX worker_budget_expected_claim_guard
    ON pg_temp.worker_budget_index_expected_guard (job_id, claim_generation)
    WHERE event_type = 'worker.budget.job_claim';
  SELECT pg_get_expr(i.indpred, i.indrelid)
  INTO expected_group_predicate
  FROM pg_index i
  WHERE i.indexrelid = 'pg_temp.worker_budget_expected_group_guard'::regclass;
  SELECT pg_get_expr(i.indpred, i.indrelid)
  INTO expected_claim_predicate
  FROM pg_index i
  WHERE i.indexrelid = 'pg_temp.worker_budget_expected_claim_guard'::regclass;
  ...
  IF group_index.relkind IS NOT NULL AND (
    ...
    RAISE EXCEPTION
      'idx_job_events_worker_budget_group_window has an unexpected definition'
      USING ERRCODE = '42804';
  END IF;
  IF group_index.relkind IS NOT NULL AND (
    group_index.indisvalid IS DISTINCT FROM TRUE
    OR group_index.indisready IS DISTINCT FROM TRUE
  ) THEN
    RAISE EXCEPTION
      'idx_job_events_worker_budget_group_window is exact but not ready and valid'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF claim_index.relkind IS NOT NULL AND (
    ...
    RAISE EXCEPTION
      'idx_job_events_worker_budget_claim_generation has an unexpected definition'
      USING ERRCODE = '42804';
  END IF;
  IF claim_index.relkind IS NOT NULL AND (
    claim_index.indisvalid IS DISTINCT FROM TRUE
    OR claim_index.indisready IS DISTINCT FROM TRUE
  ) THEN
    RAISE EXCEPTION
      'idx_job_events_worker_budget_claim_generation is exact but not ready and valid'
      USING ERRCODE = '55000';
  END IF;
END
$$;
