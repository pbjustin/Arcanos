...
function detectMalformed(output, expectedFormat) {
    const issues = [];
    let totalChecks = 0;
    let issueCount = 0;
    // Helper functions
...
function runDetectionTests() {
    ...
    for (const test of formatTests) {
        const detection = detectMalformed(test.text, test.format);
    ...
    const correctDetections = results.filter(r => expectedMalformed.includes(r.name) === r.isMalformed).length;
