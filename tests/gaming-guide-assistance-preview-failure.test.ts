...
describe('served Gaming assistance production-core proof failure boundary', () => {
  ...
  it('adds all six proof markers only after the actual fixed component assertions pass', async () => {
    ...
    for (const [header, version] of [[contract.proofHeader, contract.proofVersion], [contract.responseProofHeader, contract.responseProofVersion],
      ...
      [contract.progressRecoveryProofHeader, contract.progressRecoveryProofVersion]]) expect(response.headers[header]).toBe(version);
  ...
  it.each(['allocation drift', 'unexpected failure'])('withholds all six Gaming markers after %s', async scenario => {
    ...
    for (const header of [contract.proofHeader, contract.responseProofHeader, contract.documentProofHeader, contract.durableRagProofHeader, contract.guideAssistanceProofHeader, contract.progressRecoveryProofHeader]) expect(response.headers[header]).toBeUndefined();
