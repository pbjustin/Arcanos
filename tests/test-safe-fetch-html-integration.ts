 ...
 */
export async function processWebContent(url: string): Promise<{ content: string; status: 'success' | 'error' }> {
  const result: SafeFetchHtmlResult = await safeFetchHtml(url);
 ...
 */
export async function batchProcessUrls(urls: string[]): Promise<Array<{ url: string; success: boolean; message: string }>> {
