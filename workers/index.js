...
if (WORKER_LOGIC !== "arcanos") {
  console.warn(
    `[AI-WORKERS] Non-standard worker logic: ${WORKER_LOGIC} - defaulting to ARCANOS`,
  );
}
...
const reportFailure = async (workerName, error) => {
  ...
  try {
    await diagnosticsService.executeDiagnosticCommand(
      `worker failure ${workerName}: ${error.message}`,
    );
  } catch (diagErr) {
...
const executeWorkerWithAIControl = async (workerName, parameters = {}) => {
  ...
  try {
    ...
    if (result.success) {
      logger.info(`AI approved execution of ${workerName}`, {
