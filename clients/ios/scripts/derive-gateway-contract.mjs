...
const operations = operationNames.map(([path, method]) => ({
  path, method, operationId: literal(property(property(property(pathObject, path), method), 'operationId'))
}));
...
function emit(name, schema) {
  ...
  const keys = fields.some(field => field.key !== field.name)
    ...
    : '';
  declarations.set(name, `public struct ${name}: Codable, Equatable, Sendable {\n${properties}\n\n    public init(${argumentsText}) {\n${assignments}\n    }\n${keys}}\n`);
}
...
const snapshot = JSON.stringify({ source: sourcePath, openapi: '3.1.0', operations, schemas }, null, 2) + '\n';
const outputs = [
