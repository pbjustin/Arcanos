...
| --- | --- |
| Documentation only | `npm run docs:check`, `npm run docs:links -- --local-only`, and `git diff --check`; inspect generators before running them. |
...
- For an exact Jest file, use `node scripts/run-jest.mjs --runTestsByPath <actual-file> --coverage=false`; a filename pattern is not proof that the intended suite ran.
...
- `npm run reindex` rewrites `backend-index.json`, `cli-agent-index.json`, `docs/BACKEND_INDEX.md`, and `docs/CLI_AGENT_INDEX.md` together. Regenerate all four after structural moves/deletions; `reindex:check` compares generated Markdown as well as JSON, so do not hand-edit these outputs. See `docs/DOCUMENTATION.md` for generator scope and audit-artifact exclusions.
