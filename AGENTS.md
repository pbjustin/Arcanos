...
- Normal Railway web/worker deployments use `node scripts/start-railway-service.mjs`, `ARCANOS_PROCESS_KIND=web|worker`, and `/readyz` for activation readiness. `/healthz` remains process liveness and `/health` remains bounded diagnostics. Native PR previews use the versioned `node scripts/start-railway-service.mjs --pr-preview-app-safe-v1` override: the web role receives an exact credential-empty child environment and imports only the contained synthetic generic-jobs application plus sealed Research and Backstage storyline fixture-selector surfaces; the worker remains passive and denies both paths. The Research surface directly exercises `src/shared/researchRequest.ts`. The storyline `POST /backstage/storyline-contract` surface uses server-owned `lifecycle-exact` and `payload-over` fixtures to exercise the real validator, response selector, and repository transaction helper against a new per-request in-memory query adapter. This is a component E2E: it does not reach PostgreSQL or prove SQL-engine locking or atomicity; the PostgreSQL 18 CI suite remains authoritative for those properties. Neither surface imports or invokes the normal route, confirmation middleware, hub, provider/fetcher, database connection, memory, or persistence effects; caller requests contain only a sealed fixture name, confirmation is not attempted, and protected effects remain disabled. `--pr-preview-safe` remains the explicit health-only fallback. This repository boundary protects trusted PRs from accidental effects; it does not protect secrets from malicious PR code, so untrusted/fork previews require provider-level secret isolation before execution. `Procfile` does not define the canonical Railway start path.
