...
- Normal Railway web/worker deployments use `node scripts/start-railway-service.mjs`, `ARCANOS_PROCESS_KIND=web|worker`, and `/readyz` for activation readiness. `/healthz` remains process liveness and `/health` remains bounded diagnostics. Native PR previews use the versioned `node scripts/start-railway-service.mjs --pr-preview-app-safe-v1` override: the web role receives an exact credential-empty child environment and imports only the contained synthetic generic-jobs application, while the worker remains passive. `--pr-preview-safe` remains the explicit health-only fallback. This repository boundary protects trusted PRs from accidental effects; it does not protect secrets from malicious PR code, so untrusted/fork previews require provider-level secret isolation before execution. `Procfile` does not define the canonical Railway start path.
