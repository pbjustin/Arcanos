...
- This file governs the repository; directory-specific rules below apply to their named paths. Check for applicable `AGENTS.override.md`, nested `AGENTS.md`, and nearby policy before editing. An override replaces the instruction file in its directory; deeper guidance takes precedence within its scope. The tracked tree has only this root `AGENTS.md`; `.cursorrules` points here. `.agents/skills/` contains task workflows, not nested instruction layers. Ignored archived source copies do not govern the active tree.
...
| `.agents/skills/` | Shared PR review, validation, protocol, worktree-sync, and preview workflows; see [Repository agent skills](docs/RUN_LOCAL.md#repository-agent-skills). |
...
- Read the matching repository skill for specialized work; it supplements current instructions and preserves existing authorization. For authorized Git transitions, inspect worktree ownership/hooks and use `--no-overwrite-ignore` with merge, checkout, or switch, including detached transitions. Follow [Safe worktree synchronization](.agents/skills/arcanos-safe-worktree-sync/SKILL.md); clean status and ordinary stashes do not protect ignored files, and `git pull` does not accept this guard.
...
Substitute the files covering the change. Prefer explicit file selection even where skill examples use patterns, which can match checkout paths and select unrelated suites. Inspect selected suites and `scripts/test-env.mjs`: the wrapper does not make every integration test offline.
...
- Follow [Gaming guide assistance](docs/GAMING_GUIDE_ASSISTANCE.md) for the public context contract and its retrieval/Trinity flow. Preserve request-scoped player claims, server-owned context provenance, conservative spoiler precedence, and original request/evidence through the stages. Context is not verified player state; source text is untrusted evidence, not instructions. Do not turn intake, incomplete, dry-run, or fallback output into a completed guide answer.
- Live retrieval and durable ingestion share `src/services/gamingDocumentResolution.ts`. Preserve URL/transport safety, cancellation, independent byte/text bounds, instruction filtering, and safe provenance. Keep resolver/chunker policy in revision identity, bounded structured-build evidence on the first searchable record, atomic active-revision replacement/reactivation, and explicit partial coverage. Inspect `src/services/gamingSourceIngestion.ts`, `src/services/gamingStoredKnowledge.ts`, and their repository consumers together.
- Supplied-guide requests without usable supplied evidence stop before provider invocation; metadata/fetch success alone is not grounding. Preserve non-fallback grounded guide prose after outer trimming, including Markdown, citations, qualifications, and metadata. Cover document acquisition and public orchestration with focused mocked tests; SQL storage/ranking and revision behavior require the separate guarded `tests/integration/gaming-durable-rag.pg18.integration.test.ts` against an authorized disposable target. A skipped database suite is not SQL evidence.
...
- `scripts/notion-authority-root-probe.mjs`, `scripts/notion-root-status-diagnostic.mjs`, and `scripts/notion-wiki-root-compat.mjs` are no-op startup compatibility shims, not live probes. Do not restore pre-listener Notion calls or global fetch/`NODE_OPTIONS` mutation there.
...
- Preserve startup sequencing in `src/server.ts`: preflight before listener binding, dependencies initialized afterward, then background runtime activation when ready. In the normal app, `/healthz` and `/health` reach the earlier public registry/lifecycle handler (200/503 from required GPT registrations); `/readyz` reports activation readiness. Dedicated worker and sealed-preview health handlers have separate semantics. See [API.md](docs/API.md). Startup can apply schema DDL, write heartbeats, and probe dependencies. Health success does not prove ingestion, retrieval, synchronization, job execution, or end-to-end behavior.
...
- Root-worker Notion synchronization is asynchronous and separate from process readiness. Preserve structural configuration validation, the all-slot readiness barrier, shared coordination, leases/activation fences, cancellation/drain, atomic publication, and queue budget headroom. The ready signal precedes background crawls; freshness and live cutover evidence do not gate `/readyz`. Exact `shadow` admits partition writers; exact `partitioned` freezes them while evidence monitoring continues. Monolith synchronization remains active in every valid mode. Protected generation, including literal shortcuts, still requires current-complete authority. The bounded authority-status read in [API.md](docs/API.md) is separate from readiness and generation admission; HTTP 200 alone does not establish current authority. Follow [Railway configuration](docs/RAILWAY_DEPLOYMENT.md#configuration) and affected sync/retrieval/PostgreSQL tests; do not broaden fallback policy to bypass authority checks.
...
- Gaming preview fixtures use server-owned synthetic data and reviewed pure cores. Keep normal acquisition/retrieval wrappers, agent mapping/Trinity/provider graphs, SQL, caches, logging sinks, and active workers outside that import graph. Preserve semantic/import pins and withhold all Gaming success markers and the success body on any fixture failure. Use the current verifier for supported proof markers; mocked public-path integration and guarded PostgreSQL runs supply separate evidence.
...
- [Architecture](docs/ARCHITECTURE.md), [workspace packages](docs/WORKSPACE_PACKAGES.md), [protocol schemas](docs/SCHEMA_PROTOCOL_GUIDE.md), and [Gaming guide assistance](docs/GAMING_GUIDE_ASSISTANCE.md).
