...
| `Procfile` | Historical process declaration for web/worker. | `railway.json` + `scripts/start-railway-service-with-integrity.mjs`, which gates the role launcher with explicit `ARCANOS_PROCESS_KIND`. | Deployment | Medium: if any Heroku/Procfile workflow exists, deletion changes startup. | Verified `railway.json` and Dockerfile use the integrity wrapper; `Procfile` bypasses it. | Extend `validate:railway`, confirm no Procfile deploy path, deployment owner approval. | Past target: 2026-06-15 | Owner decision overdue; delete candidate after proof |
...
| Python backend job status/result compatibility method names (`request_gpt_job_status` and `request_gpt_job_result`) | Method names preserve an older client API, but their transport has been migrated. | Capability-bound canonical `GET /jobs/:id` and `GET /jobs/:id/result`. | Python backend client | Low while compatibility names remain thin aliases over canonical routes. | `daemon-python/arcanos/backend_client/chat.py` uses only `/jobs/*`, retains `jobReadToken` response metadata, and sends the explicit token or transient `ARCANOS_JOB_READ_TOKEN` only as `x-arcanos-job-read-token`; daemon tests assert the canonical paths and header. | Keep the route, token-validation, header-carrier, and negative backend-operation routing tests while the compatibility method names remain. | Migration completed before 2026-07-23 | Transport migration complete; method names retained for compatibility |
...
- [Reusable-code refactor audit](docs/audits/reusable-code/2026-04-29/refactor-audit.md)
