...
| Python backend job status/result compatibility method names (`request_gpt_job_status` and `request_gpt_job_result`) | Method names preserve an older client API, but their transport has been migrated. | Canonical `GET /jobs/:id` and `GET /jobs/:id/result`. | Python backend client | Low while compatibility names remain thin aliases over canonical routes. | `daemon-python/arcanos/backend_client/chat.py` uses only `/jobs/*` for these reads, and daemon tests assert the canonical paths. | Keep the route assertions and negative backend-operation routing tests while the compatibility method names remain. | Migration completed before 2026-07-23 | Transport migration complete; method names retained for compatibility |
