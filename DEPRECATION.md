...
| `Procfile` | Historical process declaration for web/worker. | `railway.json` + `scripts/start-railway-service.mjs` with explicit `ARCANOS_PROCESS_KIND`. | Deployment | Medium: if any Heroku/Procfile workflow exists, deletion changes startup. | Verified `railway.json` and Dockerfile use the launcher; `Procfile` bypasses it. | Extend `validate:railway`, confirm no Procfile deploy path, deployment owner approval. | Past target: 2026-06-15 | Owner decision overdue; delete candidate after proof |
