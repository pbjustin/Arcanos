...
router.get("/finetune-status", async (req, res) => {
  ...
  try {
    const { fineTuneRoutingService } = await import(
      "../services/finetune-routing"
    );

    const isActive = await fineTuneRoutingService.isFineTuneRoutingActive(
      userId,
      sessionId,
    );
    const statusMessage = await fineTuneRoutingService.getStatusMessage(
      userId,
      sessionId,
    );
    ...
    sendErrorResponse(
      res,
      500,
      "Failed to get fine-tune routing status",
      error.message,
    );
  }
});
  ...
  try {
    ...
    if (result.success) {
      ...
      sendErrorResponse(
        res,
        500,
        result.error || "AI dispatcher error",
        `Echo: ${message}`,
      );
    }
  } catch (error: any) {
