...
function normalizeLabelFromPrompt(prompt: string): string {
  //audit Assumption: label must be safe for inclusion in system prompts; failure risk: prompt injection via stored label; expected invariant: label contains only safe printable chars and is reasonably short; handling strategy: whitelist characters and truncate.
  const compact = prompt.trim().replace(/\s+/g, ' ');
  if (!compact) {
    return 'intent_system';
  }
  // Allow letters, numbers, basic punctuation and whitespace. Strip control characters and other symbols.
  const sanitized = compact.replace(/[^\w\s\-\.,:;()\/\\@#&]+/g, '').trim();
