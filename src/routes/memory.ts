...
const getContainerId = (req: Request): string => {
  return (
    (req.headers["x-container-id"] as string) ||
  ...
  try {
    ...
    if (memory_value === undefined) {
      return sendErrorResponse(
        res,
        400,
        "memory_value is required (can be null)",
        'Example: { memory_key: "user_preference", memory_value: { theme: "dark" } }',
      );
    }
    ...
    const result = useDatabase
      ...
      : await fallbackMemory.storeMemory(
          container_id,
          "default",
          "context",
          memory_key,
    ...
    arcanosLogger.memorySnapshot("Error saving memory", {
      error: error.message,
    });
...
router.get("/thread/:id", async (req: Request, res: Response) => {
  try {
    ...
    const thread = useDatabase ? (result as any).memory_value : result;

    if (
      !thread ||
      (typeof thread === "object" && Object.keys(thread).length === 0)
    ) {
      return res.status(404).json({ error: "Thread not found", id });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to load thread", details: error.message });
  }
});
