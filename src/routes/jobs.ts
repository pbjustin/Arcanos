...
import {
  ...
  type GptJobResultLookupPayload,
...
export function isBackstageBookerBearerReadableJob(
  ...
  return job?.job_type === 'gpt'
    && job.idempotency_scope_hash === expectedScopeHash
...
export async function readBackstageBookerAsyncResult(
...
): Promise<GptJobResultLookupPayload> {
  ...
  input.signal?.throwIfAborted();
  if (!isBackstageBookerBearerReadableJob(initialJob, expectedScopeHash)) {
    return buildGptJobResultLookupPayload(input.jobId, null);
  }
  ...
  if (
    ...
    const getOwnedJobById = async (jobId: string): Promise<JobData | null> => {
      ...
      return isBackstageBookerBearerReadableJob(candidate, expectedScopeHash)
        ? candidate
        : null;
    };
  ...
  input.signal?.throwIfAborted();
  if (!isBackstageBookerBearerReadableJob(selectedJob, expectedScopeHash)) {
    return buildGptJobResultLookupPayload(input.jobId, null);
  }

  return buildGptJobResultLookupPayload(
    input.jobId,
...
const router = express.Router();
router.use(createBackstageBookerAsyncResultRouter());
router.use(createGenericJobsRouter({
