...
function detectEmbeddedMcpAction(body: unknown): McpControlAction | null {
  ...
  const payloadRecord = isRecord(normalizedBody.payload)
    ? normalizedBody.payload
    : normalizedBody;
  const embeddedEnvelope = isRecord(payloadRecord.mcp)
    ? payloadRecord.mcp
    : payloadRecord;
  const envelopeAction =
    getString(embeddedEnvelope, 'action') ??
    getString(embeddedEnvelope, 'operation');
