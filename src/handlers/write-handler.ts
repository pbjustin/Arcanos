  ...
  async handleWriteRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (!contentValidation.isValid) {
        console.warn('⚠️ WRITE-VALIDATION: Rejecting request with invalid content:', contentValidation.reason);
        res.status(400).json({
      ...
      if (this.isMalformedResponse(result)) {
        console.warn('🚨 MALFORMED-RESPONSE: Detected malformed model response');
      ...
      if (!finalValidation.isValid) {
        console.error('❌ WRITE-FINAL: Response content invalid after processing');
        result = fallbackHandler.injectFallbackContent(result, message);
      }
      
      console.log('✅ WRITE-COMPLETE: Request processed successfully with content validation');
      res.json({
        ...result,
