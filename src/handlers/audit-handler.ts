...
export class AuditHandler {
  ...
  async handleAuditRequest(req: Request, res: Response): Promise<void> {
    ...
    console.log(
      "🔍 AuditHandler: Audit endpoint triggered - logging audit activity:",
      timestamp,
    );
    ...
    try {
      ...
      if (this.isMalformedAuditResponse(result)) {
        console.warn(
          "🚨 MALFORMED-AUDIT-RESPONSE: Detected malformed audit response",
        );
  ...
  private isMalformedAuditResponse(response: any): boolean {
    ...
    return (
      ...
      response.auditResult === null ||
      response.auditResult === undefined ||
      (typeof response.auditResult === "string" &&
        response.auditResult.trim() === "") ||
      response.success === undefined
    );
  }

  private injectFallbackAuditContent(
    response: any,
  ...
  private logMalformedAuditResponse(
    response: any,
    request: any,
    timestamp: string,
  ): void {
    const logEntry = {
      ...
      response_structure: {
        ...
        audit_result_length: response.auditResult
          ? response.auditResult.length
          : 0,
  ...
  getMalformedAuditLogs(): any[] {
    return this.auditActivityLog.filter(
      (log) => log.type === "malformed_audit_response",
    );
  }
