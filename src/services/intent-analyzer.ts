...
export type IntentType = 'WRITE' | 'AUDIT' | 'UNKNOWN';
...
export class IntentAnalyzer {
  ...
  analyzeIntent(input: string): IntentAnalysisResult {
    ...
    // Check for strong phrase indicators first
    ...
    const totalScore = narrativeScore + auditScore;
    ...
    if (totalScore === 0) {
      ...
      intent = 'WRITE';
      confidence = Math.min(narrativeScore / (narrativeScore + auditScore), 0.95);
      reasoning += `Narrative score: ${narrativeScore}, Audit score: ${auditScore}.`;
    } else if (auditScore > narrativeScore) {
      intent = 'AUDIT';
      confidence = Math.min(auditScore / (narrativeScore + auditScore), 0.95);
      reasoning += `Audit score: ${auditScore}, Narrative score: ${narrativeScore}.`;
    } else {
      // Tied scores - default to UNKNOWN with low confidence
      intent = 'UNKNOWN';
      confidence = 0.3;
      reasoning += `Tied scores (${narrativeScore}/${auditScore}) - unclear intent.`;
    }
