...
function maskBackstageQuotedRequestText(value: string): string {
...
function splitBackstageRequestClauses(value: string): string[] {
  return maskBackstageQuotedRequestText(value)
...
function shouldUseBoundedBackstageReviewMode(prompt: string): boolean {
  ...
  const clauses = splitBackstageRequestClauses(normalizedPrompt)
  ...
  const hasEmbeddedCreativeGerundRequest = BACKSTAGE_EMBEDDED_CREATIVE_GERUND_PATTERN.test(
    maskBackstageQuotedRequestText(normalizedPrompt)
  );
