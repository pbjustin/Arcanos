...
interface BackstageCompactOutputContract {
...
interface BackstageCompactRetryEmbeddedContentState {
...
function resolveBackstageCompactOutputContract(
  prompt: string,
  tokenLimit: number
...
function parseBackstageBookerCompactRetryNumberedParagraphs(
  output: string
): string[] | null {
  const items: string[] = [];
...
function isBackstageBookerCompactRetryOutputValid(
  output: string,
  contract: BackstageCompactOutputContract
): boolean {
  const { itemPolicy, wordBounds } = contract;
  ...
  return items.every(item => countWords(item) <= wordBounds.wordsPerItem);
}
...
export async function generateBooking(
  ...
  try {
    ...
    let usedCompactOutputRetry = false;
    try {
