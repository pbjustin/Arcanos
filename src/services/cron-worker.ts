...
export function initCronWorker(): void {
  ...
  cron.schedule("*/15 * * * *", async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log(
          "[AI-CRON] Health check denied by AI:",
          result.error || "No approval",
        );
      }
    } catch (error: any) {
  ...
  cron.schedule("0 */6 * * *", async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log(
          "[AI-CRON] Maintenance denied by AI:",
          result.error || "No approval",
        );
      }
    } catch (error: any) {
  ...
  cron.schedule("0 */4 * * *", async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log(
          "[AI-CRON] Memory sync denied by AI:",
          result.error || "No approval",
        );
      }
    } catch (error: any) {
  ...
  cron.schedule("*/30 * * * *", async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log(
          "[AI-CRON] Goal watcher denied by AI:",
          result.error || "No approval",
        );
      }
    } catch (error: any) {
  ...
  cron.schedule("15,45 * * * *", async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log(
          "[AI-CRON] Assistant sync denied by AI:",
          result.error || "No approval",
        );
      }
    } catch (error: any) {
      console.error(
        "[AI-CRON] AI assistant sync control error:",
        error.message,
      );
    }
  });
  ...
  async function executeAssistantSync(): Promise<void> {
    workerStatusService.updateWorkerStatus(
      "worker-assistant-sync",
      "running",
      "ai_approved_assistant_sync",
    );
    ...
    try {
      ...
      console.log(
        `[AI-ASSISTANT-SYNC] Successfully synced ${assistantCount} assistants`,
      );
      console.log(
        "[AI-ASSISTANT-SYNC] Available assistants:",
      ...
      workerStatusService.updateWorkerStatus(
        "worker-assistant-sync",
        "idle",
        `ai_assistant_sync_complete_${assistantCount}_assistants`,
      );
    } catch (error: any) {
      console.error(
        "[AI-ASSISTANT-SYNC] Assistant sync failed:",
        error.message,
      );
      workerStatusService.updateWorkerStatus(
        "worker-assistant-sync",
        "error",
        "ai_assistant_sync_failed",
      );
    }
  }
