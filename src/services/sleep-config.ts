...
export function getCurrentSleepWindowStatus(): SleepWindowStatus {
  ...
  if (easternHour < sleepStart) {
    nextSleepStartET = nowET.set({
      hour: sleepStart,
      minute: 0,
      second: 0,
      millisecond: 0,
    });
    nextSleepEndET = nowET.set({
      hour: sleepEnd,
      minute: 0,
      second: 0,
      millisecond: 0,
    ...
    nextSleepStartET = nowET
      .plus({ days: 1 })
      .set({ hour: sleepStart, minute: 0, second: 0, millisecond: 0 });
    nextSleepEndET = nowET.set({
      hour: sleepEnd,
      minute: 0,
      second: 0,
    ...
    nextSleepStartET = nowET
      .plus({ days: 1 })
      .set({ hour: sleepStart, minute: 0, second: 0, millisecond: 0 });
    nextSleepEndET = nowET
      .plus({ days: 1 })
      .set({ hour: sleepEnd, minute: 0, second: 0, millisecond: 0 });
  }

  const timeUntilSleep = inSleepWindow
    ? undefined
    : Math.max(0, Math.round(nextSleepStartET.diff(nowET, "minutes").minutes));
  const timeUntilWake = inSleepWindow
    ? Math.max(0, Math.round(nextSleepEndET.diff(nowET, "minutes").minutes))
    : undefined;
...
export function logSleepWindowStatus(): void {
  ...
  if (status.inSleepWindow) {
    ...
    console.log(
      `[SLEEP-WINDOW] ⏰ Wake up in ${status.timeUntilWake} minutes at ${status.nextSleepEnd?.toLocaleString()}`,
    );
  } else {
    console.log(`[SLEEP-WINDOW] 🌅 Currently awake (outside 7 AM - 2 PM ET)`);
    console.log(
      `[SLEEP-WINDOW] ⏰ Sleep in ${status.timeUntilSleep} minutes at ${status.nextSleepStart?.toLocaleString()}`,
    );
  }
}
