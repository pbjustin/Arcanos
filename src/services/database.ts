...
export class DatabaseService {
  ...
  constructor() {
    ...
    if (!databaseUrl) {
      console.warn(
        "⚠️ DATABASE_URL not configured, memory service will use fallback in-memory storage",
      );
  ...
  private async connectWithRetry(): Promise<void> {
    while (this.connectionRetryCount < this.maxRetries && !this.isConnected) {
      try {
        ...
        const delay =
          this.retryDelay * Math.pow(2, this.connectionRetryCount - 1); // Exponential backoff

        console.log(
          `⚠️ Database connection attempt ${this.connectionRetryCount}/${this.maxRetries} failed: ${error.message}`,
        );

        if (this.connectionRetryCount >= this.maxRetries) {
          console.error(
            "❌ Max database connection retries exceeded. Running in degraded mode.",
          );
          break;
        }
  ...
  async saveMemory(request: SaveMemoryRequest): Promise<MemoryStateEntry> {
    ...
    try {
      ...
      if (this.isRecoveryError(error)) {
        console.log(
          "🔄 Database appears to be in recovery, attempting reconnection...",
        );
