...
function archiveInput(url: string): { parsed: URL; identifier: string | null } | null {
  try {
...
export function recognizeGamingArchiveItem(url: string): string | null {
  return archiveInput(url)?.identifier ?? null;
}
...
function assertNotAborted(options: FetchAndCleanOptions): void {
...
function assertRawDocument(
  document: FetchAndCleanRawDocument | undefined,
  kind: "metadata" | "text"
): asserts document is FetchAndCleanRawDocument {
...
export async function resolveGamingArchiveResource(
  ...
  const item = archiveInput(url);
  if (!item) return null;
  if (!item.identifier) throw new GamingArchiveResolutionError("INVALID_ITEM_URL");
  const identifier = item.identifier;
  assertNotAborted(options);
  let metadataDocument: FetchAndCleanRawDocument | undefined;
  try {
    await fetchAndClean(`https://archive.org/metadata/${identifier}`, 0, {
      ...options,
      includeLinks: false,
  ...
  const preparedDerivative = prepareGamingResourceUrl(derivativeUrl);
  if (!preparedDerivative || preparedDerivative.privateFetchUrl !== derivativeUrl) {
    throw new GamingArchiveResolutionError("UNSAFE_STORAGE_LOCATION");
  }
  let document: FetchAndCleanRawDocument | undefined;
  let text: string;
  try {
    text = await fetchAndClean(derivativeUrl,
