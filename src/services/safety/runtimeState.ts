...
interface QuarantineInput {
  ...
  dedupeKey?: string;
    ...
    function normalizeCounters(value: unknown): SafetyCounters {
      ...
      if (isRecord((value as any).heartbeatMisses)) {
        for (const [key, raw] of Object.entries((value as any).heartbeatMisses)) {
      ...
      if (isRecord((value as any).healthyCycles)) {
        for (const [key, raw] of Object.entries((value as any).healthyCycles)) {
          const parsed = Number(raw);
          if (Number.isFinite(parsed) && parsed >= 0) {
      ...
      return {
        duplicateSuppressions: Number.isFinite(Number((value as any).duplicateSuppressions))
          ? Math.max(0, Math.floor(Number((value as any).duplicateSuppressions)))
          : 0,
        quarantineActivations: Number.isFinite(Number((value as any).quarantineActivations))
          ? Math.max(0, Math.floor(Number((value as any).quarantineActivations)))
          : 0,
        workerFailures: normalizeWorkerFailures((value as any).workerFailures),
        heartbeatMisses,
        healthyCycles
      };
    }
    ...
    function normalizeUnsafeCondition(raw: unknown): UnsafeConditionRecord | null {
      ...
      if (typeof (raw as any).conditionId !== 'string' || typeof (raw as any).code !== 'string' || typeof (raw as any).message !== 'string') {
        return null;
      }

      return {
        conditionId: (raw as any).conditionId,
        code: (raw as any).code as UnsafeConditionCode,
        message: (raw as any).message,
        blocking: (raw as any).blocking !== false,
        createdAt: typeof (raw as any).createdAt === 'string' ? (raw as any).createdAt : new Date().toISOString(),
        monotonicTsMs: Number.isFinite(Number((raw as any).monotonicTsMs))
          ? Math.floor(Number((raw as any).monotonicTsMs))
          : getMonotonicTimestampMs(),
        quarantineId: typeof (raw as any).quarantineId === 'string' ? (raw as any).quarantineId : undefined,
        metadata: isRecord((raw as any).metadata) ? (raw as any).metadata : undefined,
        clearedAt: typeof (raw as any).clearedAt === 'string' ? (raw as any).clearedAt : undefined,
        clearedBy: typeof (raw as any).clearedBy === 'string' ? (raw as any).clearedBy : undefined,
        clearNote: typeof (raw as any).clearNote === 'string' ? (raw as any).clearNote : undefined
      };
    }
    ...
    function normalizeQuarantine(raw: unknown): SafetyQuarantineRecord | null {
      ...
      if (typeof (raw as any).quarantineId !== 'string' || typeof (raw as any).kind !== 'string' || typeof (raw as any).reason !== 'string') {
        return null;
      }

      return {
        quarantineId: (raw as any).quarantineId,
        kind: (raw as any).kind as QuarantineKind,
        reason: (raw as any).reason,
        integrityFailure: Boolean((raw as any).integrityFailure),
        autoRecoverable: (raw as any).autoRecoverable !== false,
        createdAt: typeof (raw as any).createdAt === 'string' ? (raw as any).createdAt : new Date().toISOString(),
        monotonicTsMs: Number.isFinite(Number((raw as any).monotonicTsMs))
          ? Math.floor(Number((raw as any).monotonicTsMs))
          : getMonotonicTimestampMs(),
        cooldownUntilMs: Number.isFinite(Number((raw as any).cooldownUntilMs))
          ? Math.floor(Number((raw as any).cooldownUntilMs))
          : undefined,
        metadata: isRecord((raw as any).metadata) ? (raw as any).metadata : undefined,
        releasedAt: typeof (raw as any).releasedAt === 'string' ? (raw as any).releasedAt : undefined,
        releasedBy: typeof (raw as any).releasedBy === 'string' ? (raw as any).releasedBy : undefined,
        releaseNote: typeof (raw as any).releaseNote === 'string' ? (raw as any).releaseNote : undefined
      };
    }
    ...
    function readStateFromDisk(): SafetyRuntimeSnapshot {
      try {
        ...
        const conditions = Array.isArray((parsed as any).conditions)
          ? ((parsed as any).conditions as unknown[])
              .map(normalizeUnsafeCondition)
              .filter((item): item is UnsafeConditionRecord => item !== null)
          : [];
        const quarantines = Array.isArray((parsed as any).quarantines)
          ? ((parsed as any).quarantines as unknown[])
        ...
        return {
          ...
          conditions,
          quarantines,
          counters: normalizeCounters((parsed as any).counters),
          trustedHashes: isRecord((parsed as any).trustedHashes)
            ? Object.entries((parsed as any).trustedHashes).reduce<Record<string, string>>((accumulator, entry) => {
    ...
    function saveState(reason: string): void {
      // Non-blocking coalesced persistence to avoid sync I/O DoS risks
      scheduleSave(reason);
    }
    ...
    function canAcceptEntityKey(key: string): boolean {
      ...
      return existing.size < MAX_ENTITY_KEYS;
    }
    ...
    export function resetSafetyRuntimeStateForTests(): void {
      ...
      try {
        ...
        // swallow cleanup errors for tests
      }
    }
