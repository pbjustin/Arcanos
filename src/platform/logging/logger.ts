...
/**
 * Recursively sanitizes potentially sensitive fields and values.
 */
export function sanitize(data: unknown, depth = 0, maxDepth = 10): any {
  if (depth > maxDepth) {
    return '[max depth reached]';
  }

  if (typeof data === 'string') {
    if (SENSITIVE_VALUE_PATTERNS.some(pattern => pattern.test(data))) {
      return '[REDACTED]';
    }
    return data;
  }
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    const isSensitive = SENSITIVE_KEYS.some(k => keyLower.includes(k.replace('-', ''))) || 
                       SENSITIVE_VALUE_PATTERNS.some(p => p.test(keyLower));
