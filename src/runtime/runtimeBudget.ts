...
export const WATCHDOG_LIMIT_MS = readIntegerEnv('WATCHDOG_LIMIT_MS', 60_000);
 ...
 * Edge case: can return zero after deadline or within safety buffer.
 */
export function getSafeRemainingMs(budget: RuntimeBudget): number {
  ...
  const safeRemaining = (budget.hardDeadline - Date.now()) - budget.safetyBuffer;
