...
function sessionKey(ctx: McpRequestContext, args: any): string {
...
function stripConfirmationFields(args: any) {
  if (!args || typeof args !== 'object') return args;
  const { confirmationNonce, confirmed, ...rest } = args as any;
  return rest;
}

function requireNonceOrIssue(args: any, toolName: string, ctx: McpRequestContext, bindTo: unknown) {
...
function notExposed(toolName: string, ctx: McpRequestContext) {
  return mcpError({
    code: 'ERR_DISABLED',
...
function wrapTool(toolName: string, ctx: McpRequestContext, handler: (args: any) => Promise<any>) {
