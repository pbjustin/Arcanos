...
import { recoverOutput, recoverGameGuide, isMalformed } from '../src/utils/output-recovery';
// Example from problem statement: Fallback for partial guides
async function handlePartialGuideExample(output, res) {
    ...
    if (output.includes("[") && !output.includes("]")) {
        ...
        const repaired = await recoverGameGuide("Fetch Baldur's Gate 3 prologue guide", output);
...
class EnhancedGuideHandler {
    static async handleGuideRequest(req, res) {
...
async function getMockGuideFromMemory(guideId) {
    return {
        id: guideId,
        sections: [
...
function formatMockGuide(guide) {
...
export { handlePartialGuideExample, withFallbackExample, EnhancedGuideHandler, runExamples };
