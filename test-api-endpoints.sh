...
CONTROL_PLANE_ACCESS_TOKEN="${ARCANOS_CONTROL_PLANE_ACCESS_TOKEN:-}"
...
if [ -z "$CONTROL_PLANE_ACCESS_TOKEN" ]; then
  echo "ARCANOS_CONTROL_PLANE_ACCESS_TOKEN is required for protected codebase API checks"
  exit 1
fi
...
curl -fsS -H "Authorization: Bearer $CONTROL_PLANE_ACCESS_TOKEN" "$BASE_URL/api/codebase/tree" -o "$TREE_RESPONSE_FILE"
...
curl -fsS -H "Authorization: Bearer $CONTROL_PLANE_ACCESS_TOKEN" "$BASE_URL/api/codebase/file?path=README.md&startLine=1&endLine=5" -o "$FILE_RESPONSE_FILE"
...
TRAVERSAL_STATUS="$(curl -sS -H "Authorization: Bearer $CONTROL_PLANE_ACCESS_TOKEN" -o "$TRAVERSAL_RESPONSE_FILE" -w "%{http_code}" "$BASE_URL/api/codebase/file?path=../package.json")"
if [ "$TRAVERSAL_STATUS" != "400" ]; then
  echo "Expected path traversal request to fail with 400, got $TRAVERSAL_STATUS"
  exit 1
fi

node -e "const fs=require('fs'); const payload=JSON.parse(fs.readFileSync(process.argv[1], 'utf8')); if (payload?.status !== 'error') process.exit(1); if (payload?.message !== 'Unable to read file') process.exit(1);" "$TRAVERSAL_RESPONSE_FILE"
